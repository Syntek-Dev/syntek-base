...
# CLAUDE.md — workflows/10-debugging-with-logs/
...
- **Definition of done:** root cause identified and documented; a regression test
  added via `code/workflows/07-debug/`; a staging/prod incident filed as a bug report
...
- **Observational only** — this workflow finds the cause; the fix and its regression
  test go through `code/workflows/07-debug/`.
