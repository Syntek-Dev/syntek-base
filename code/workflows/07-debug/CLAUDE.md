...
debugging (`how-to/workflows/03-debugging/`) and log-based debugging (workflow `10`).
...
- **Routing:** execute via `STEPS.md`, typically with the `debug` or `bugfix`
  ...
  `how-to/workflows/03-debugging/`, then fix the logic here. Trace the fault with the
...
- **Keep the fix minimal** — do not refactor surrounding code in the same commit; if
  the fix reveals a design problem, open a separate task for workflow `08`.
