...
- `code/workflows/01-implement-story/CONTEXT.md` — implement one `US###` story, backend and frontend
...
- `code/docs/FORWARD-VOICE.md` — the sibling of that rule, one seam further out: **GATE-REPORTING governs what a check may claim it looked at, this governs what a document may claim exists.** Every path a shipped file names is a claim about two trees — this one and the generated project it will be read in — and the two do not hold the same files. Carries the one rule (a citation that does not resolve is fixed, never marked), the two directions and the mechanism each takes: **Direction A** absent here and present downstream, answered by a row in `how-to/src/PROJECT-PATHS.md` naming what creates it; **Direction B** present here and excluded by copier, declared on the line with `doc-references: template-only`. States plainly which half `audits/doc-references.sh` cannot decide
