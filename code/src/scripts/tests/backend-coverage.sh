#!/usr/bin/env bash
#
# backend-coverage.sh — Run the Django/pytest suite with coverage reporting.
#
...
#
# Enforces the 75% line and branch coverage floor (hard failure if below).
#
...
if ! docker compose -f "$COMPOSE_FILE" ps backend-test --status running | grep -q "running"; then
  printf 'backend-coverage.sh error: backend-test container is not running.\n' >&2
