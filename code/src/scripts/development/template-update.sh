...
log "    bash code/src/scripts/tests/all.sh                        # nothing broke"
log "    git commit -m 'chore(template): update from <template>'"
log ""
log "  Run the suites before committing even when no dependency changed: an update"
log "  moves code and configuration, not only manifests. This is the same habit the"
log "  dependency updater states after its own --apply."
