...
├── health.sh                ← probe /health/ and /health/ready/; report what the word means
...
| `health.sh`             | Probe both health endpoints and say what the answer means — `--watch` polls across one cache TTL, because a single readiness reading can predate the fault. Diagnosis only; it restarts nothing. Recovery is `how-to/docs/HEALTH-PROBES.md`                                                                                                                                                                              |
