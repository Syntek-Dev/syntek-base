...
Beyond everything below, a test also declares the story it exists for — see _Markers_.
...
A pytest test runs under a **tier** marker and declares the **story** it exists for. The tier
marker is assigned by path at collection — `code/src/scripts/tests/CONTEXT.md` owns that mapping —
while the story marker is the one an author writes. `--strict-markers` is on, so an unregistered
marker is an error rather than a silent no-op: all four are registered in the root
`pyproject.toml`, under `[tool.pytest.ini_options]` → `markers`.

| Marker                        | Declares                                        | Read by                                                                  |
| ----------------------------- | ----------------------------------------------- | ------------------------------------------------------------------------ |
| `@pytest.mark.unit`           | Pure unit test, no database access              | `code/src/scripts/tests/backend.sh -m unit`                              |
| `@pytest.mark.integration`    | Needs a real database                           | `code/src/scripts/tests/backend.sh -m integration`                       |
| `@pytest.mark.e2e`            | Browser test — never runs in the ordinary suite | `code/src/scripts/tests/e2e-py.sh`                                       |
| `@pytest.mark.story("US###")` | The user story the test exists for              | `code/src/scripts/tests/test-record.sh` — the per-story record generator |

**The first three select what runs. The fourth selects nothing.** Its purpose is the per-story
test record in `project-management/src/18-TESTS/`: the suites run whole-project and their JUnit
XML carries no story attribution, so the record is generated by reading this marker rather than by
filtering on a path or a branch diff — a path breaks the moment a story spans two apps, and a diff
misses every test a later fix touches. Never reach for a story-filtered run; the marker is
attribution, not selection.

```python
import pytest

pytestmark = pytest.mark.story("US012")  # inherited by every test in this module


@pytest.mark.story("US019")  # repeatable — this behaviour is load-bearing for US019 too
@pytest.mark.django_db
def test_signup_rejects_a_duplicate_email() -> None: ...
```

- **Inheritable** — a module-level or class-level `pytestmark` covers everything beneath it, so a
  story's test module declares it once rather than per test.
- **Repeatable** — stack the decorator where more than one story genuinely tests the behaviour;
  the test then appears in each of their records.
- **Warn-only** — `bash code/src/scripts/audits/story-markers.sh` lists unmarked tests and exits
  `0`, because failing it would block every shared helper and every test predating the convention.
  The consequence is silence rather than a red run: **an unmarked test is silently absent from its
  story's record**, so run the audit before trusting a short one.

Bruno's half of the same convention is the `tags:` entry in a request's `meta` block —
