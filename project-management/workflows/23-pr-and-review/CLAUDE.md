...
- **Concrete steps:** confirm tests green and linters clean → review against
  ...
  `workflows/22-implementation-documentation`) are complete, and write the code-review
...
- **Documentation hard gate:** the implementation records and `CONTEXT.md`/`CLAUDE.md`
  closeout (authored in `workflows/22-implementation-documentation`) must be complete
...
- **Hand-written:** `STEPS.md`, `CHECKLIST.md`; the PR-stage review record
  `REVIEW-US###-*.md` in `src/19-REVIEWS/` and the test records in `src/18-TESTS/`. The
  GDPR/security/QA/SEO/API implementation records are authored in
  `workflows/22-implementation-documentation`, not here.
