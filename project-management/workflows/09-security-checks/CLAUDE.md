...
# CLAUDE.md — workflows/09-security-checks/

Read order: `.claude/CLAUDE.md` → `.claude/MEMORY.md` → this folder's `CONTEXT.md`
(when-to-use, prerequisites, frameworks — imported above) → this file → `STEPS.md`
then `CHECKLIST.md`.
...
`src/09-SECURITY/` before a line of code is written.
...
- **Routing:** run `STEPS.md` in order; drive the heavier analysis with the
  ...
  signed-off wireframes, and the GDPR review (`workflows/08-gdpr-compliance`).
...
- **Concrete steps:** read `docs/SECURITY-GUIDE.md` → threat-model each flow/wireframe
  ...
  `src/09-SECURITY/` under the right sub-dir (`THREAT-MODEL/`, `ASSESSMENTS/`,
...
- Threat modelling gates sprint planning — proceed to `workflows/14-sprint-plans/`
  only once blocking findings are cleared and QA (`workflows/10-qa-checks`) is next.
- Instructional `.md` files ≤ 300 code lines.

## Output & naming

- **Hand-written:** `STEPS.md`, `CHECKLIST.md`; findings under `src/09-SECURITY/`
