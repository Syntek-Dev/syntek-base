...
### Verify — authored in `21-implementation-documentation` (do not write here)

If any applicable record below is missing or incomplete, return to
`21-implementation-documentation` before proceeding — do not write it here.
...
- [ ] Code review record written (`REVIEW-US###-<descriptor>[-DD-MM-YYYY].md` → `src/18-REVIEWS/`)
- [ ] Test status record written (`US###-TEST-STATUS.md` → `src/17-TESTS/`)
- [ ] Manual testing guide written (`US###-MANUAL-TESTING.md` → `src/17-TESTS/`)

### Record quality

- [ ] Findings record present for the story (`src/19-FINDINGS/`) — written even when nothing was found
- [ ] Every applicable IMPLEMENTATION record (from `21`) verified present and complete before merge
- [ ] Any newly discovered Critical or High findings escalated to `src/10-SECURITY/VULNERABILITIES/IMPLEMENTATION/` and fed back to `20`
...
- [ ] Documentation closeout from `21-implementation-documentation` (CONTEXT/CLAUDE updates + code-review-graph refresh) confirmed complete
