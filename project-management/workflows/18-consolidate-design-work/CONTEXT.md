...
```text
project-management/workflows/18-consolidate-design-work/
...
coherent system. Workflows `02`–`15` run per story, so each story produces its own
...
- Every story in the planning cycle has cleared `17-story-plans/`
- Before `19-backend-code/` — no implementation starts from unconsolidated design
