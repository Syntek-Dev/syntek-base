...
| Step      | Section                                                                       |
| --------- | ----------------------------------------------------------------------------- |
| All steps | **Internal — Live Artefacts** → src/14-DECISIONS/                             |
| All steps | src/14-DECISIONS/CLAUDE.md — immutability, monotonic indices, authoring rules |
| Template  | src/14-DECISIONS/ADR-000-TEMPLATE.md — the five-section scaffold to copy      |
...
- [ ] `src/14-DECISIONS/` has been scanned for an existing ADR this one might
      supersede
...
List `src/14-DECISIONS/` and find the highest existing `ADR-###` index. The next
...
Copy `src/14-DECISIONS/ADR-000-TEMPLATE.md` to
`src/14-DECISIONS/ADR-###-<TITLE>.md`, with `TITLE` in `SCREAMING-SNAKE-CASE`
