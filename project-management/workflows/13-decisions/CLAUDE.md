...
# CLAUDE.md — workflows/13-decisions/
...
as an immutable `ADR-###-<TITLE>.md` in `src/13-DECISIONS/`, using
...
- **Routing:** run `STEPS.md` in order; the hard gates —
  `src/13-DECISIONS/CLAUDE.md` (immutability, monotonic indices) and
  `ADR-000-TEMPLATE.md` (the five-section scaffold) — must be read before Step 1.
  Inputs: the driving story or spec (`03-database-schema`, `09-security-checks`,
  `12-api-design`, or any 01–12 workflow) that surfaced the trade-off.
...
- **Definition of done:** the ADR is the single source of truth for the decision;
  `workflows/14-sprint-plans/` and `workflows/15-story-plans/` cite it as a constraint
