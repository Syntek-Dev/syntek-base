...
| Step      | Section                                                                       |
| --------- | ----------------------------------------------------------------------------- |
| All steps | **Internal — Live Artefacts** → src/13-DECISIONS/                             |
| All steps | src/13-DECISIONS/CLAUDE.md — immutability, monotonic indices, authoring rules |
| Template  | src/13-DECISIONS/ADR-000-TEMPLATE.md — the five-section scaffold to copy      |

---

## Prerequisites

- [ ] A driving user story (`src/01-STORIES/US###.md`) or specify-tier spec exists —
...
- [ ] The relevant specify-tier workflow has surfaced the choice (e.g.
      `03-database-schema` for a schema shape, `09-security-checks` for an
      auth/session strategy, `12-api-design` for a contract-shaping decision)
- [ ] `src/13-DECISIONS/` has been scanned for an existing ADR this one might
      supersede
...
List `src/13-DECISIONS/` and find the highest existing `ADR-###` index. The next
...
Copy `src/13-DECISIONS/ADR-000-TEMPLATE.md` to
`src/13-DECISIONS/ADR-###-<TITLE>.md`, with `TITLE` in `SCREAMING-SNAKE-CASE`
