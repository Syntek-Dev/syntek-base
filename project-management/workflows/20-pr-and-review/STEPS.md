...
| Impl records | Verified here, authored in `workflows/19-implementation-documentation/` → src/08-GDPR/, src/09-SECURITY/, src/10-QA/, src/11-SEO/, src/12-API-DESIGN/ |
...
Set the story `**Status:**` to `In Review` in `src/01-STORIES/US###.md` and commit — the lefthook
regenerates the ClickUp export and the `clickup-sync` workflow pushes it to the board. Status set
per `project-management/docs/SPRINT-PLANNING-GUIDE.md` → Story Statuses.
...
The design/compliance IMPLEMENTATION records are **written in `19-implementation-documentation`**, which runs before this workflow. Here you **verify** each applicable one exists and is complete, then **write** the two PR-stage records that are the outputs of Steps 1–2.

**Verify (authored in `19-implementation-documentation`)** — if any applicable record is missing or incomplete, return to `19` before proceeding; do not write it here:

| Discipline                                          | Required                                           | Expected in                                  |
| --------------------------------------------------- | -------------------------------------------------- | -------------------------------------------- |
| GDPR review                                         | Always                                             | `src/08-GDPR/IMPLEMENTATION/`                |
| Security (assessment / audit / threat model / vuln) | Always                                             | `src/09-SECURITY/<CATEGORY>/IMPLEMENTATION/` |
| QA plan                                             | Always                                             | `src/10-QA/IMPLEMENTATION/`                  |
| SEO review                                          | Only if story adds public-facing pages             | `src/11-SEO/IMPLEMENTATION/`                 |
| API design                                          | Only if story adds or changes the Django Ninja API | `src/12-API-DESIGN/IMPLEMENTATION/`          |
| Findings                                            | Always                                             | `src/18-FINDINGS/`                           |
...
| ------------------ | -------- | -------------------------------------------------- | ----------------- |
| Code review record | Always   | `REVIEW-US###-<descriptor>[-DD-MM-YYYY].md`        | `src/17-REVIEWS/` |
| Test record        | Always   | `US###-TEST-STATUS.md` + `US###-MANUAL-TESTING.md` | `src/16-TESTS/`   |

Any newly discovered Critical or High findings during review must be escalated to
`src/09-SECURITY/VULNERABILITIES/IMPLEMENTATION/` immediately and fed back to
...
**Hard gate — complete before closing this story.** The primary `CONTEXT.md`/`CLAUDE.md` updates and the code-review-graph refresh are done in `19-implementation-documentation` — confirm they are complete. Then, if **this** workflow created new files, directories, or established new constraints:
