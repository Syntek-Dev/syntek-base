...
# CLAUDE.md — workflows/20-pr-and-review/

Read order: `.claude/CLAUDE.md` → `.claude/MEMORY.md` → this folder's `CONTEXT.md`
(when-to-use, prerequisites, hard gate — imported above) → this file → `STEPS.md`
then `CHECKLIST.md`.
...
- **Concrete steps:** confirm tests green and linters clean → review against
  ...
  `workflows/19-implementation-documentation`) are complete, and write the code-review
...
- **Definition of done:** PR reviewed and merged per the branch chain; the implementation
  records (from `19`) verified complete and the review/test records written; version
...
- **Documentation hard gate:** the implementation records and `CONTEXT.md`/`CLAUDE.md`
  closeout (authored in `workflows/19-implementation-documentation`) must be complete
...
- **Hand-written:** `STEPS.md`, `CHECKLIST.md`; the PR-stage review record
  `REVIEW-US###-*.md` in `src/17-REVIEWS/` and the test records in `src/16-TESTS/`. The
  GDPR/security/QA/SEO/API implementation records are authored in
  `workflows/19-implementation-documentation`, not here.
