...
# CLAUDE.md — workflows/02-sprint-planning/
...
- **Routing:** run `STEPS.md` against `CHECKLIST.md`; use `sprint`.
  **Hard gate:** read `docs/SPRINT-PLANNING-GUIDE.md` before writing — the MoSCoW
...
- **Concrete steps:** confirm a story backlog exists in `src/01-STORIES/` and the prior
  sprint is closing → pick candidate stories with MoSCoW (Must/Should/Could/Won't) →
  write the goal and scope as `SPRINT-##.md` in `project-management/src/02-SPRINTS/`.
- **Definition of done:** `SPRINT-##.md` records intent (goal + candidates); the
  definitive plan is deferred to `workflows/14-sprint-plans/` after the pre-sprint
...
- **This record captures intent only.** Detailed story assignment, phase breakdown, and
  GDPR/security/QA constraints belong to `workflows/14-sprint-plans/` — keep the two
  distinct.
- Check candidate stories against open GDPR (`src/08-GDPR/`), security
  (`src/09-SECURITY/`), and QA (`src/10-QA/`) findings before committing scope.
- Documentation only — no code.

## Output & naming

- **Hand-written:** the sprint record and `STEPS.md`/`CHECKLIST.md` updates.
- High-level records `SPRINT-##.md` in `src/02-SPRINTS/`; detailed plans
  `SPRINT-PLAN-##.md` live under `src/14-SPRINT-PLANS/`; dates DD/MM/YYYY.
