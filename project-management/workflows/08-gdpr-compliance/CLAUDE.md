...
# CLAUDE.md — workflows/08-gdpr-compliance/
...
- **Routing:** run `STEPS.md` against `CHECKLIST.md`; use `gdpr`.
...
- **Concrete steps:** confirm the feature is implemented and data flows are understood →
  trace each data touchpoint from `src/04-USER-FLOW/` to a lawful basis and retention
  ...
  (`DJANGO-LOGGING.md`) → record the outcome under `project-management/src/08-GDPR/`.
...
- **Hand-written:** planning gap reports under `src/08-GDPR/PLANNING/`; post-
  implementation records `GDPR-IMPL-US###-*.md` under `src/08-GDPR/IMPLEMENTATION/`;
