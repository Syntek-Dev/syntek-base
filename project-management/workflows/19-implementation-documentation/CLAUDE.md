...
# CLAUDE.md — workflows/19-implementation-documentation/
...
record the story's findings in `src/18-FINDINGS/`, before the PR is raised in
`20-pr-and-review`.

## How to work here

- **Routing:** run `STEPS.md` in order; drive with the `doc-writer` agent (Opus). It runs
  after the code workflows (`16-backend-code`, `17-api-code`, `18-frontend-code`) and
  before `20-pr-and-review`. Read `code/docs/CODE-REVIEW-GRAPH.md` before touching docs.
...
- **Concrete steps:** identify which specs applied to the story → copy each applicable
  ...
  the findings record from `src/18-FINDINGS/FINDING-US000-TEMPLATE.md` → update the touched
...
- **This workflow writes the records; `20-pr-and-review` only verifies them** — do not
...
- **Findings are recorded, never fixed here** — the record states the smallest fix; the fix
  lands in a later story, `src/19-BUGS/`, or `src/20-REFACTORING/`. Where a migration, index,
...
- **Hand-written:** `STEPS.md`, `CHECKLIST.md`; the per-story IMPLEMENTATION records under
  `src/08-GDPR/IMPLEMENTATION/`, `src/09-SECURITY/<CATEGORY>/IMPLEMENTATION/`,
  `src/10-QA/IMPLEMENTATION/`, `src/11-SEO/IMPLEMENTATION/`,
  `src/12-API-DESIGN/IMPLEMENTATION/`, each linked to its `US###`; and the per-story findings
  record under `src/18-FINDINGS/`.
