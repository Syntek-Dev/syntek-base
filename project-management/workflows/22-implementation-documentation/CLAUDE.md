...
record the story's findings in `src/20-FINDINGS/`, before the PR is raised in
`23-pr-and-review`.
...
- **Concrete steps:** identify which specs applied to the story → copy each applicable
  ...
  the findings record from `src/20-FINDINGS/FINDING-US000-TEMPLATE.md` → update the touched
