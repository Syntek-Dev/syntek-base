...
| Step             | Section                                                                           |
| ---------------- | --------------------------------------------------------------------------------- |
| All steps        | **Internal — Live Artefacts** → src/16-STORY-PLANS/                               |
| All steps        | src/16-STORY-PLANS/CLAUDE.md — copy-the-template rules, dependency DAG honesty    |
| Template         | src/16-STORY-PLANS/STORY-PLAN-US000-TEMPLATE.md — the canonical superset scaffold |
| Gathering inputs | **Internal — Live Artefacts** → src/15-SPRINT-PLANS/, src/14-DECISIONS/           |

---

## Prerequisites

- [ ] Story slotted into a sprint (`src/15-SPRINT-PLANS/##-SPRINT-PLAN-##.md`)
- [ ] Every 02–13 spec relevant to this story exists and is signed off (GDPR, security,
      QA, SEO, API design as applicable)
- [ ] Any ADR this story rests on is `Accepted` in `src/14-DECISIONS/`
...
- The sprint plan (`src/15-SPRINT-PLANS/`) — goal, priority, and phase assignment for this
  story
- Every ADR (`src/14-DECISIONS/`) the story rests on
- Every relevant 02–13 spec: story, schema, user flow, wireframes, GDPR, security, QA, SEO,
  API design
...
Copy `src/16-STORY-PLANS/STORY-PLAN-US000-TEMPLATE.md` to
`src/16-STORY-PLANS/STORY-PLAN-US###-<SCREAMING-KEBAB-DESC>.md`. Never start from scratch.
...
(raise a new ADR in `src/14-DECISIONS/` — via `workflows/14-decisions/` — if the story
...
| -------- | ------------------ | ----------------------------------------------------------------------------------- |
| Backend  | `18-backend-code`  | Models, services, migration (TDD via `code/workflows/02-tdd-cycle/`)                |
| API      | `19-api-code`      | Django Ninja routers, endpoints, request/response schemas, permission checks        |
| Frontend | `20-frontend-code` | Templates, django-components, HTMX partials — component-library reuse checked first |
...
Copy the obligations from the 02–13 specs into the plan's GDPR, Security, and Testing
...
1. Save to `src/16-STORY-PLANS/STORY-PLAN-US###-<SCREAMING-KEBAB-DESC>.md`
2. Add the row to `src/16-STORY-PLANS/CONTEXT.md` → Plans Index (file link, story, Status)
3. Reference the plan in the driving user story (`src/02-STORIES/US###.md`)
4. Proceed to `workflows/18-backend-code/` to begin implementation
