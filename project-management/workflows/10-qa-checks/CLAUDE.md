...
# CLAUDE.md — workflows/10-qa-checks/
...
in `src/10-QA/` before any code is written.
...
- **Routing:** run `STEPS.md` in order; drive scenario generation with the
  ...
  completed security checks (`workflows/09-security-checks`). QA planning is pre-code,
...
- **Concrete steps:** read `docs/QA-GUIDE.md` → map scenarios against
  `src/01-STORIES/` and wireframes → cover the security findings from
  `src/09-SECURITY/` and WCAG 2.2 AA checks → write `QA-US###-<DESCRIPTION>.md` into
  `src/10-QA/PLANNING/` → feed edge cases back into story acceptance criteria → satisfy
  `CHECKLIST.md`.
...
- **Hand-written:** `STEPS.md`, `CHECKLIST.md`; QA plans under `src/10-QA/PLANNING/`
