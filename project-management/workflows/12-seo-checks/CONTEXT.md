...
that `21-frontend-code` implements against and `22-implementation-documentation` verifies. The
...
| Building the page to those targets                                | `21-frontend-code`                |
| Auditing the built page, Lighthouse, the `IMPLEMENTATION/` record | `22-implementation-documentation` |

## When to use this

**Entry condition: the story's `SEO` flag is not `N/A`.** The flag is set at
`02-story-creation` from the feature map's slice row, and it means the story adds or changes a public page. A story whose
`SEO` flag reads `N/A` skips this gate, and every downstream checklist reads the flag
...
- Before `15-decisions` closes the story's loop

## When NOT to use this

- To verify a built page — that is `22-implementation-documentation`
...
- `project-management/workflows/22-implementation-documentation/` — verifies these targets on the
