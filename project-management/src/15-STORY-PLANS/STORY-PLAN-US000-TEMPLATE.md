...
═══════════════════════════════════════════════════════════════════════════════
  • Copy to STORY-PLAN-US###-<SCREAMING-KEBAB-DESCRIPTOR>.md, replace every <...> and
    [PLACEHOLDER], and add the row to 15-STORY-PLANS/CONTEXT.md "Plans Index".
  ...
  • Status: pick ONE value from the canonical ClickUp board vocabulary in the
    ...
    workflow — defined in `project-management/docs/SPRINT-PLANNING-GUIDE.md`
...
<!-- > Source authority: where this plan conflicts with `project-management/src/01-STORIES/US###.md`
       or `…/14-SPRINT-PLANS/##-SPRINT-PLAN-##.md`, the §"…" of that doc wins. GDPR authority:
       `…/08-GDPR/…`. This plan records the engineering route, not the requirement. -->
...
<!-- ◇ Keep when the story makes or depends on a non-trivial architectural choice.
     ...
     `project-management/src/13-DECISIONS/ADR-###-*.md` and link it in the header. -->
...
<!-- ◇ Keep if API in scope. Follows code workflow 04-api-design + PM workflow 12-api-design. -->
...
> Where the project keeps a cross-cutting programme plan (`15-STORY-PLANS/PLAN-<DESCRIPTOR>.md`,
...
- **Retention:** [period + trigger — align with `08-GDPR/RETENTION-DELETION.md`].
...
- **DPIA:** [required? if yes → `08-GDPR/DPIA-<DESCRIPTOR>.md`].
...
> Findings/assessments are written to `project-management/src/09-SECURITY/**` (see §Documentation
...
> by the story's scope table; always-on rows apply regardless. PM workflows 01–15 (design/planning)
...
| ------------------------------- | ------------------------------------------------------------------- | -------- |
| Story written + AC agreed       | `project-management/workflows/01-story-creation/`                   | ✓ always |
| Sprint slotted                  | `…/02-sprint-planning/`                                             | ✓        |
| DB schema signed off            | `…/03-database-schema/`                                             | ✓ / —    |
| User flow mapped                | `…/04-user-flow-design/`                                            | ✓ / —    |
| Brand / components / wireframes | `…/05-brand-guides/`, `…/06-component-designs/`, `…/07-wireframes/` | ✓ / —    |
| GDPR review                     | `…/08-gdpr-compliance/`                                             | ✓ / —    |
| Security review                 | `…/09-security-checks/`                                             | ✓ / —    |
| QA plan                         | `…/10-qa-checks/`                                                   | ✓        |
| SEO check (public pages)        | `…/11-seo-checks/`                                                  | ✓ / —    |
| API contract designed           | `…/12-api-design/`                                                  | ✓ / —    |
| Detailed sprint plan            | `…/14-sprint-plans/`                                                | ✓        |
| Backend code                    | `…/16-backend-code/`                                                | ✓ / —    |
| API code                        | `…/17-api-code/`                                                    | ✓ / —    |
| Frontend code                   | `…/18-frontend-code/`                                               | ✓ / —    |
| PR & review                     | `…/20-pr-and-review/`                                               | ✓ always |
| Release                         | `…/21-release/`                                                     | ✓ / —    |
...
- [Steps a human follows — captured in `project-management/src/16-TESTS/US###-MANUAL-TESTING.md`.]
...
| ------------------------------------------ | ------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------ | -------------------------- |
| **This plan**                              | `STORY-PLAN-US###-<DESC>.md`                                        | `project-management/src/15-STORY-PLANS/`                                                         | planning                                                                 | Always                     |
| ADR (if a cross-cutting decision)          | `ADR-###-<title>.md`                                                | `…/src/13-DECISIONS/`                                                                            | planning                                                                 | Conditional                |
| User story                                 | `US###.md`                                                          | `…/src/01-STORIES/`                                                                              | PM `01-story-creation`                                                   | Always                     |
| Sprint plan                                | `##-SPRINT-PLAN-##.md`                                              | `…/src/14-SPRINT-PLANS/`                                                                         | PM `14-sprint-plans`                                                     | Always                     |
...
> `12-API-DESIGN` each split into `PLANNING/` (pre-dev plan/spec, per story) + `IMPLEMENTATION/`
> (delivery write-back, per story) — `09-SECURITY` nests that split under its four categories. There
...
> workflow numbers only up to **12**; from `src/13-DECISIONS` on they diverge — do not assume
> `src/14-SPRINT-PLANS`=WF14 (WF14 is `16-backend-code`).
...
| ---------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `project-management/src/15-STORY-PLANS/CONTEXT.md`   | Add this plan's row to the Plans Index (file link, story, Status); bump **Last Updated**                                                                            |
...
| --- | ------------------------------ | ------------------------------------------------------------------------ | ------------------------------------------------------------ |
| 1   | **Story (source of truth)**    | `project-management/src/01-STORIES/US###.md` → `**Status:**` header      | Edit here **first** — the only file the ClickUp export reads |
| 2   | **This plan**                  | `…/src/15-STORY-PLANS/STORY-PLAN-US###-*.md` → `Status` (metadata table) | Keep in lockstep with the story                              |
| 3   | **Plans Index**                | `…/src/15-STORY-PLANS/CONTEXT.md` → Plans Index `Status` column          | Also bump the index **Last Updated**                         |
| 4   | **Sprint record**              | `…/src/02-SPRINTS/SPRINT-##.md` → story status table                     | If the story is listed in a sprint                           |
| 5   | **Sprint plan**                | `…/src/14-SPRINT-PLANS/##-SPRINT-PLAN-##.md` → story table / DoD         | If covered by a detailed sprint plan                         |
...
> (`15-STORY-PLANS/PLAN-<DESCRIPTOR>.md`) where one exists.
...
- [ ] PR raised and promoted via PM `20-pr-and-review/`; review record in `17-REVIEWS/`
...
- [ ] (If releasing) version bumped + changelog per PM `21-release/` and `VERSIONING-GUIDE.md`
