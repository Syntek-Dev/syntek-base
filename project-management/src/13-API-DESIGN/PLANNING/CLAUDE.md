...
# CLAUDE.md — src/13-API-DESIGN/PLANNING/
...
- **Routing:** designs are produced by `project-management/workflows/13-api-design/` using
  the `planner` agent, against a story in `../../02-STORIES/` and its agreed schema in
...
- **Documentation only** — no implementation code (`code/src/django/`), no migration SQL
  or ERDs (`../../04-DATABASE/`), no user stories (`../../02-STORIES/`); no secrets or
  `.env` content.
