...
- **Routing:** plans are produced by `project-management/workflows/10-qa-checks/` (after
  security checks and wireframe sign-off) using the `qa-tester` agent, against a story in
  `../../01-STORIES/` and its wireframe, governed by `project-management/docs/QA-GUIDE.md`.
...
- **Documentation only — no source, secrets, or `.env` content.** Automated results and
  manual guides live downstream in `../../16-TESTS/`; do not duplicate them here.
