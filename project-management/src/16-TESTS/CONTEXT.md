# project-management/src/16-TESTS
...
`13-DECISIONS → 14-SPRINT-PLANS → 15-STORY-PLANS → code → 16-20 records`: the story plan
(15) is the master a developer codes from, and the 16–20 folders (tests, reviews, findings, bugs,
...
(backend/API/frontend) and `project-management/workflows/20-pr-and-review/`. Keep both in
step with the story and its suites; append the update date on every change.

## Cross-references

- `../01-STORIES/` — the user stories these records test
- `../15-STORY-PLANS/` — the per-story implementation plan the tests close the loop on
- `../10-QA/` — the QA plan (PLANNING) and review (IMPLEMENTATION) these records feed
