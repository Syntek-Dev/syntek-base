...
record, tied to a story at both ends, mirroring the 08-GDPR split.
...
(that role is served by the per-story plans, as in 08-GDPR). A story with no public URL
...
- `project-management/workflows/11-seo-checks/` — produces the `PLANNING/` plan and audits
- `project-management/workflows/20-pr-and-review/` — where the `IMPLEMENTATION/` record lands
