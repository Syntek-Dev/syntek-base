...
- **Routing:** this is the PM-side record; the refactor itself is executed under `code/` via
  ...
  workflow `22-pr-and-review`. Draft it with **Opus** — an implementation record, not a
  planning artefact.
- **Concrete steps:** copy `REFACTORING-US000-TEMPLATE.md` →
  ...
  `../16-STORY-PLANS/` plan, and the driving review → record the verification results.
