...
**Register re-reconciled**: 16/08/2026 — `TEMPLATE-GAPS.md` still carries **no uncharted entries** and root `GAPS.md`/`DEFERRED.md` are still empty. **None of N-032 to N-040 came from a register**: five were routed here by `MAP-ABSENCE`, four were measured on 16/08. The _Register claimed_ table does not account for them, as it does not for N-026 and N-027
**Status**: Charting
...
> `how-to/src/TEMPLATE-GUIDE/TEMPLATE-GAPS.md`, which **is** tracked. This file is therefore the
...
Done when Frontier and Fog of war are both empty and `TEMPLATE-GAPS.md` carries standing
limitations only.
...
The register here is `how-to/src/TEMPLATE-GUIDE/TEMPLATE-GAPS.md`, not the root `GAPS.md` —
the root register ships, so the template's own items were never allowed in it. Root `GAPS.md`
and `DEFERRED.md` were both read on 13/08/2026 and hold **no open entries**, which is why the
unrelated count is zero.

**This is a claim, not a close.** `workflows/21-implementation-documentation/` closes against
shipped code.
...
- **`Routing skills resolve` is on the target set and deliberately held back.** It is N-030 — a gate
  ...
  means nothing is the instrument outrunning the rule, so the row carries `Hold` until N-030 lands.
  **N-030 landed at `0c22b79` on 16/08/2026 and the row was not flipped** — the condition is met
  and the text still reads `Hold`, with the reasoning above still in the present tense. Left that
  way on purpose: see the residue bullet in the N-030 verdict.
...
- **Residue, and it is a real open loop: the gate's promotion did not happen with the fix.**
  `git/PR-AND-REQUIRED-CHECKS.md:106` still reads `Routing skills resolve` → **`Hold — see
below`**, and lines 118–122 still describe the parser defect **in the present tense** as the
  reason for the hold. The node's own stated exit condition was _"the row carries `Hold` until
  N-030 lands"_ — it has landed. Deliberately **not** flipped here: the other half of promoting a
  check is a GitHub branch-protection setting no file in this repository can make, so flipping the
  row alone would put a written claim ahead of the enforcement, which is the class this batch is
  named for. It belongs to whoever holds the required-set change — **which is now `N-057`, charted
  16/08/2026.** Both citations above drifted by four lines under `33926e9` and were re-anchored in
  the same pass: the residue is a settled node's live remainder sitting inside another node's
  table, which the collision table now records.
...
| Node  | Decision                                                                                                                         | Type     | Blocked by | Blocking a story? |
| ----- | -------------------------------------------------------------------------------------------------------------------------------- | -------- | ---------- | ----------------- |
| N-042 | **New.** `doc-references.sh` is green on a developer's disk and red on a fresh checkout                                          | task     | none       | no                |
| N-046 | **New.** Three scripts report an action they did not take, or a scope they did not honour                                        | task     | none       | no                |
| N-052 | **New.** `doc-references.sh` cannot see a bare script name, so the citations N-044 leaves behind are ungoverned                  | grilling | none       | no                |
| N-055 | **New.** Four gates skip a leg they cannot run and still print success — "could not look" is filed as "looked, and it was clean" | grilling | none       | no                |
| N-056 | **New.** Six gates print a green verdict over a population of zero, and `sync-trees.sh` has no exit case for "matched nothing"   | grilling | none       | no                |
| N-057 | **New.** A required check no pull-request event can produce, and a table calling ten already-flipped rows a promotion target     | grilling | none       | no                |
| N-058 | **New.** N-010 residue — two bare `pre-pr-check.sh` invocations survive its fix, one of them in a skill an agent executes        | task     | none       | no                |
...
**N-057 — the required set holds the half of a stated pair that cannot report, and the guide
naming it went stale 2h17m after it was written.** Measured at HEAD `a9c56a1`, 16/08/2026,
...
- **Two bare invocations, and the census is exhaustive rather than indicative.** A sweep for every
  ...
  `how-to/workflows/06-quality-gates/CHECKLIST.md:49` and `.claude/skills/git/SKILL.md:75`.
- **Both were observed exiting 0, not reasoned about.** `bash .claude/hooks/pre-pr-check.sh < /dev/null`
  prints nothing and returns **EXIT=0** having run none of the eight gates, because `:30` reads
  ...
  `STEPS.md:124-130` has them right. A report auditing citation drift carried it.
...
- **The skill site is worse, and it was not in N-010's field of view.** `.claude/skills/git/SKILL.md:75`
...
| Node  | Decision                                                                                                                                                                | Type     | Blocked by                                                   | Blocking a story? |
| ----- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------- | ------------------------------------------------------------ | ----------------- |
| N-044 | ~~The health caller has two names, and the second is still uncommitted~~ **Premise, subject and type all dead. What survives is naming hygiene on an unwritten script** | task     | none                                                         | no                |
| N-048 | **New.** The Prometheus scrape job has four homes and two spellings, and the file disclaiming ownership states one anyway                                               | task     | none                                                         | no                |
| N-049 | **New.** N-016's comment rule contradicts itself across its two homes — a **resolved** node, live again                                                                 | task     | none                                                         | no                |
| N-051 | **New.** The version rule has three homes naming three file sets, and the one that runs is not the one claiming to be canonical                                         | grilling | none — **paired with N-050** (_Unbatched_), settled together | no                |
...
- **It carries its own remedy, and the remedy corrects the map.** `doctrine-drift.sh` **can** hold
  this instance — `code/docs` is in `SCAN_DIRS` (`doctrine-drift.sh:52-60`) and both `-web` homes
  are fenced code (`nix` at `:95`, `yaml` at `:229`), so the audit is green only because nobody
  added a claim row anchored on `job_name.*ORG_SLUG`. **This map's twice-stated verdict that the
  audit named for Batch D cannot reach a Batch D rule (MAP:474-478, MAP:1096-1105) is too
  pessimistic by one counter-example**, and this node is where that gets written down.
...
**N-051 — three homes for the version rule, and the one that runs is not the one claiming to be
canonical.** Paired with **N-050** (_Unbatched_) and settled with it: N-051 is the cause, N-050 is
...
| Node  | Decision                                                                                                                                             | Type     | Blocked by                                                                                                                                                                       | Blocking a story? |
| ----- | ---------------------------------------------------------------------------------------------------------------------------------------------------- | -------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------- |
| N-020 | Write `deploy.sh`, `rollback.sh`, `health-check.sh`. **Re-type to a split proposed 16/08, not applied**                                              | task     | image-publish workflow (absent) · ~~the caller's owner (N-044)~~ **re-worded 16/08 — the name is settled, `health-check.sh` is unblocked** · `/scale-planning` for `rollback.sh` | no                |
...
| N-031 | Shipped guides cite copier-**excluded** paths — resolves here, dangles in every generated project                                                    | grilling | **N-043** (the rule it would classify against is false) · **N-042** in practice (its own PR is blocked at a required check)                                                      | no                |
| N-037 | ~~Three claims~~ **all three fixed 16/08 by N-010. What remains is the floor beneath them**                                                          | task     | **none at the remainder** — the `stack-django:168` half stays bound to N-026                                                                                                     | no                |
| N-043 | **New.** The shipped exclusion register is false on four counts, and it is the rule N-031 would classify against                                     | task     | none                                                                                                                                                                             | no                |
| N-045 | **New.** The Bruno request template a developer copies documents a **GraphQL** API this project does not have                                        | task     | none                                                                                                                                                                             | no                |
| N-054 | **New.** The health contract publishes a dependency set it cannot probe and an endpoint nothing serves                                               | task     | none                                                                                                                                                                             | no                |
...
**N-043 — the shipped register of what does not ship is false, and N-031 cannot be scheduled
against it.** `how-to/src/TEMPLATE-GUIDE/06-GENERATION.md` Section _Excluded from rendering_
...
- `:103` lists **`how-to/src/TEMPLATE-GUIDE/`** as excluded. `copier.yml:75-81` says in terms:
  ...
  in `_exclude`.
- `:103` also lists **`how-to/src/TEMPLATE-TOKENS.md`** as excluded. It is not in `_exclude`; it
  ships.
- The _Seeded state_ row `:105` **omits `/uv.lock`**, which `copier.yml` excludes and regenerates
  by task.
- **`copier.yml:30` excludes `/copier.yml` itself**, so every shipped sentence citing the contract
  dangles downstream — a whole class the register does not mention.
- **This is Batch E in its worst form**: it ships, and it is the document a reader would use to
  decide whether a citation dangles. Typed `task` — the facts are decidable by reading `copier.yml`;
  what to do about the self-citing `copier.yml` class belongs to N-031.
- **Re-framed 16/08/2026: this is not an unnoticed defect, it is the residue of a sweep that ran
  two days later and skipped the most authoritative file.** `24a2d9e` (16/08, 17:42) is titled
  _"four files said TEMPLATE-GUIDE is excluded, and copier.yml excludes one file in it"_ and touched
...
- **Anchors settled.** `copier.yml`'s `_exclude` spans **`:29-197`** — `_exclude:` at `:29`, last
  ...
  two ways on this page (`:96-105` in one place, `:101-108` in another); the register is
  `06-GENERATION.md:98` over rows `:103-108`, and `:105` omits `/uv.lock` (`copier.yml:44`).
...
project template"_. That refusal is now doctrine at `../14-DECISIONS/CLAUDE.md`, because the offer
...
| N-047 | **New.** The `v5.0.0` copier migration is mis-keyed — the break it warns about ships at `v4.0.0`, four tags earlier          | grilling | none                                                       | no                |
| N-050 | **New.** A shipped guide asserts a version three days and eight releases dead, and was written stale                         | task     | none — **paired with N-051** (_Batch D_), settled together | no                |
| N-059 | **New.** A shipped comment bans the glob dialect four live legs in the same file rely on, and the fault is order-sensitivity | grilling | none — **the changelog half may belong to N-047**          | no                |
...
**N-050 — a shipped guide asserts a version three days and eight releases dead, and it was written
stale.** Charted 16/08/2026. Paired with **N-051** (_Batch D_) and settled with it — N-051 is the
...
- **It did not compound N-038 or N-039.** The premise — _"a release is where `shipped-readme.sh`
  ...
  `project-management/workflows/23-release/STEPS.md` contains **no audit step at all**. The
