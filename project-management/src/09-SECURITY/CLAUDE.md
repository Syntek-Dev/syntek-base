...
that gate a feature through `workflows/09-security-checks/` before code ships.

## How to work here

- **Routing:** never author here free-hand — run `workflows/09-security-checks/`
  ...
  `qa-tester` agents (Fable). `docs/SECURITY-GUIDE.md` governs STRIDE, OWASP A01–A10,
  and NIST CSF 2.0.
...
- **PLANNING/ precedes IMPLEMENTATION/** — the pre-implementation artefact must exist
  ...
  with evidence.
- **Per story** — security analysis is tied to a `US###` at both ends, like 08-GDPR;
