...
`IMPLEMENTATION/` template tied to a user story, mirroring the 08-GDPR split.
...
`PLANNING/` report folder (that role is served by the per-story plans, as in 08-GDPR).

## When to use this

Outputs from `workflows/09-security-checks/` are saved here — run after wireframes are
