...
<!-- > Source authority: where this plan conflicts with `project-management/src/02-STORIES/US###.md`
       or `…/16-SPRINT-PLANS/##-SPRINT-PLAN-##.md`, the Section "…" of that doc wins. GDPR authority:
...
| **Design tokens** (any visual value)                                                                              | `code/docs/DESIGN-TOKENS.md` — all colour/spacing/radius/type/shadow/motion from the DB-canonical token layer, consumed as `var(--token)`, never literals                                    | ✓ / —    |
...
<!-- ◇ Keep when the story makes or depends on a non-trivial architectural choice.
     ...
     `project-management/src/15-DECISIONS/ADR-###-*.md` and link it in the header. -->
...
- Reuse the django-components library (`code/src/django/components/`) **before** building new
  ...
  (`code/docs/DESIGN-TOKENS.md`), consumed as `var(--token)` — never literals.
...
<!-- ◇ Keep only if fixing pre-existing bugs en route. Mark resolved with ✅. Prefer a separate
     bugfix (code workflow 10-debug) + a `21-BUGS/BUG-<DESC>-DD-MM-YYYY.md` report if non-trivial. -->
...
> Where the project keeps a cross-cutting programme plan (`17-STORY-PLANS/PLAN-<DESCRIPTOR>.md`,
...
> by the story's scope table; always-on rows apply regardless. PM workflows 02–17 (design/planning)
> must be complete before any code; 18–22 (implementation/delivery) run inside the worktree.
...
| Detailed sprint plan            | `…/16-sprint-plans/`                                                | ✓        |
| Backend code                    | `…/19-backend-code/`                                                | ✓ / —    |
| API code                        | `…/20-api-code/`                                                    | ✓ / —    |
| Frontend code                   | `…/21-frontend-code/`                                               | ✓ / —    |
| PR & review                     | `…/23-pr-and-review/`                                               | ✓ always |
| Release                         | `…/24-release/`                                                     | ✓ / —    |
...
- `code/workflows/07-review/` clean → PM `23-pr-and-review/` (branch promotion + write back the
  implementation records in _Documentation Write-Ups_) → PM `24-release/` when cutting a version
...
| GDPR implementation record                 | `GDPR-IMPL-US###-*.md`                                              | `…/src/09-GDPR/IMPLEMENTATION/`                                                                  | PM `23-pr-and-review` (write-back)                                                         | Conditional                |
| Security audit / assessment / threat model | `AUDIT-…`, `ASSESSMENT-…`, threat-model, `VULN-…` (`-DD-MM-YYYY`)   | `…/src/10-SECURITY/{AUDITS,ASSESSMENTS,THREAT-MODEL,VULNERABILITIES}/{PLANNING,IMPLEMENTATION}/` | PM `10-security-checks` + `22-implementation-documentation` / code `08-security-hardening` | Conditional                |
| QA plan (pre-dev)                          | `QA-PLAN-US###-<DESC>.md`                                           | `…/src/11-QA/PLANNING/`                                                                          | PM `11-qa-checks`                                                                          | Always                     |
| QA implementation review                   | `QA-IMPL-US###-<DESC>.md`                                           | `…/src/11-QA/IMPLEMENTATION/`                                                                    | PM `23-pr-and-review`                                                                      | Always                     |
| SEO implementation record                  | `SEO-IMPL-US###-<DESC>-DD-MM-YYYY.md`                               | `…/src/12-SEO/IMPLEMENTATION/`                                                                   | PM `21-frontend-code` → `12-seo-checks`                                                    | Conditional (public pages) |
...
| Test status / manual testing               | `US###-TEST-STATUS.md`, `US###-MANUAL-TESTING.md`                   | `…/src/18-TESTS/`                                                                                | PM `19`–`21` build phases, finalised at `23-pr-and-review`                                 | Always                     |
...
| Release (version bump)                     | root `VERSION`, `CHANGELOG.md`, `RELEASES.md`, `VERSION-HISTORY.md` | repo root                                                                                        | PM `24-release`                                                                            | Conditional                |

> **Folder gotchas (from the workflow↔folder map):** design/compliance folders `09-GDPR`–
> `14-LOGGING` each split into `PLANNING/` (pre-dev plan/spec, per story) + `IMPLEMENTATION/`
...
> numbers (`09`–`14`) but are written **during delivery**, by `22-implementation-documentation` —
> every one of those folders says so in its own `IMPLEMENTATION/CONTEXT.md`. `src/` numbers align
> with workflow numbers **through `17`**; from `src/18-TESTS` on they diverge — do not assume
> `src/18-TESTS`=WF18 (WF18 is `18-consolidate-design-work`; the tests folder has no workflow of
> its own).
...
> (`17-STORY-PLANS/PLAN-<DESCRIPTOR>.md`) where one exists.
