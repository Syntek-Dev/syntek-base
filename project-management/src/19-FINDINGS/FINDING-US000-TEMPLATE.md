...
**Codes from:** `../16-STORY-PLANS/STORY-PLAN-US###-<DESCRIPTOR>.md` — the implementation
...
also recorded in `DEFERRED.md`) · `Bug — US###` (a defect; file in `../20-BUGS/`) ·
`Refactor` (structural debt; action in `../21-REFACTORING/`) · `ADR` (reopens a
hard-to-reverse trade-off; graduate to `../14-DECISIONS/`) · `Accepted — {reason}` (a
...
- `../16-STORY-PLANS/STORY-PLAN-US###-<DESCRIPTOR>.md` — the plan this closes the loop on
- `../18-REVIEWS/REVIEW-US###-<DESCRIPTOR>.md` — the merge-gating review from the same PR
