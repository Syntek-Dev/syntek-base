...
each story to its story plan** in `../15-STORY-PLANS/` (the code master) — per-story
implementation depth lives there, never duplicated here.

## How to work here

- **Routing:** start from `project-management/workflows/14-sprint-plans/`
  (`STEPS.md` + `CHECKLIST.md`); it governs when and how a plan is written. Sizing,
  MoSCoW, and phase-breakdown conventions come from
  `project-management/docs/SPRINT-PLANNING-GUIDE.md`. Use `sprint`
...
- **Concrete steps:** copy `00-SPRINT-PLAN-00-TEMPLATE.md` → pull stories from
  `../01-STORIES/` and the matching record in `../02-SPRINTS/SPRINT-##.md` →
  prioritise with MoSCoW → break into phases → link each story to its story plan in
  `../15-STORY-PLANS/` → fold in **sprint-wide** constraint summaries from the
  `08-GDPR`, `09-SECURITY`, `10-QA`, `11-SEO`, `12-API-DESIGN` reviews (field-level
...
- **No per-story implementation depth here.** Models, resolvers, components, and
  field-level GDPR/security belong in the story plan (`../15-STORY-PLANS/`); the sprint
  plan carries only sprint-wide summaries and the story-plans index.
- **Every plan needs a matching `../02-SPRINTS/SPRINT-##.md`** record — the two are
