...
`../15-STORY-PLANS/` — the master a developer actually codes from. Do **not** duplicate
...
**Source sprint:** `../02-SPRINTS/SPRINT-{##}.md` · **Capacity:** {N} SP · **Stories:** {count}
...
> **Source Authority**
>
> `../03-DATABASE/` is the single source of truth for all table definitions, field names,
> encryption markers, and RLS policies. `../04-USER-FLOW/` is the single source of truth
> for all user interaction flows. Field names and flow steps in `../01-STORIES/` and
> `../02-SPRINTS/` reflect the design at story-writing time and **may not match the final
> schema or flows** — where they differ, this plan and all implementation follow
> `../03-DATABASE/` and `../04-USER-FLOW/`.

## Sprint Reference Documents

The specs in scope for this sprint. Each story's **story plan** (`../15-STORY-PLANS/`)
...
| ------------------ | --------------------------------------------------------------------------------------------- |
| Sprint definition  | `../02-SPRINTS/SPRINT-{##}.md`                                                                |
| User stories       | `../01-STORIES/US{###}.md` (one per story)                                                    |
| Database           | `../03-DATABASE/` — `SCHEMA-*.md` · `ERD-*.md` · `ENCRYPTION-*.md` · `MIGRATION-NOTES-RLS.md` |
| User flows         | `../04-USER-FLOW/USER-FLOW-{AREA}.md` (list the flows this sprint touches)                    |
| Brand & components | `../05-BRAND-GUIDE/` · `../06-COMPONENTS/` (frontend stories only)                            |
| Wireframes         | `../07-WIREFRAMES/SCREENS/WF-{###}-*.html` (frontend stories only)                            |
| GDPR               | `../08-GDPR/PLANNING/GDPR-PLAN-US{###}-*.md` (per story that touches PII)                     |
| Security           | `../09-SECURITY/{CATEGORY}/PLANNING/{TYPE}-PLAN-US{###}-*.md`                                 |
| QA                 | `../10-QA/PLANNING/QA-PLAN-US{###}-*.md` (per story)                                          |
| SEO                | `../11-SEO/PLANNING/SEO-PLAN-US{###}-*.md` (public-page stories only)                         |
| API design         | `../12-API-DESIGN/PLANNING/API-PLAN-US{###}-*.md` (per story with a Django Ninja surface)     |
| Decisions          | `../13-DECISIONS/ADR-{###}-{TITLE}.md` (ADRs this sprint must honour)                         |
| **Story plans**    | `../15-STORY-PLANS/STORY-PLAN-US{###}-*.md` — **the code master for each story**              |

---

## Stories

Prioritise with MoSCoW (see `../../docs/SPRINT-PLANNING-GUIDE.md`). Reserve capacity for
...
Per-story implementation depth lives in `../15-STORY-PLANS/`, **not** in this plan. Before
...
| Story   | Story plan (`../15-STORY-PLANS/`) | Status ({Draft / Ready / In Progress / Done}) |
| ------- | --------------------------------- | --------------------------------------------- |
...
- QA: every Must/Should story has a QA plan in `../10-QA/PLANNING/`; no unresolved `AC-GAP` entries.
- SEO: {public-page stories meet the `../11-SEO/PLANNING/` targets — or "N/A, no public pages this sprint"}
...
- [ ] GDPR constraints implemented and verified per `../08-GDPR/`
- [ ] All QA scenarios passing (automated and manual) per `../10-QA/`
...
- [ ] Gaps found during the sprint recorded in `../08-GDPR/` / `../09-SECURITY/` as applicable
- [ ] Retrospective notes captured in the matching `../02-SPRINTS/SPRINT-{##}.md` (optional)
