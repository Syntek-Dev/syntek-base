# project-management/src/17-TESTS
...
`14-DECISIONS → 15-SPRINT-PLANS → 16-STORY-PLANS → code → 17-21 records`: the story plan
(15) is the master a developer codes from, and the 17–21 folders (tests, reviews, findings, bugs,
...
(backend/API/frontend) and `project-management/workflows/22-pr-and-review/`. Keep both in
step with the story and its suites; append the update date on every change.

## Cross-references

- `../02-STORIES/` — the user stories these records test
- `../16-STORY-PLANS/` — the per-story implementation plan the tests close the loop on
...
- `project-management/workflows/22-pr-and-review/` — where the records are finalised
