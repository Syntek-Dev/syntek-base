...
each story to its story plan** in `../17-STORY-PLANS/` (the code master) — per-story
implementation depth lives there, never duplicated here.

## How to work here

- **Routing:** start from `project-management/workflows/16-sprint-plans/`
...
- **Concrete steps:** copy `00-SPRINT-PLAN-00-TEMPLATE.md` → pull stories from
  ...
  `../17-STORY-PLANS/` → fold in **sprint-wide** constraint summaries from the
...
- **No per-story implementation depth here.** Models, resolvers, components, and
  field-level GDPR/security belong in the story plan (`../17-STORY-PLANS/`); the sprint
