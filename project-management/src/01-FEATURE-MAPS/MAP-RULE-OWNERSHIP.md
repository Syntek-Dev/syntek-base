...
**Status**: Charting
**Frontier open**: 18 · **Blocking open**: 0 · **Resolved**: 1
...
| Defect classes             | Reuses `GAPS.md`'s five: **A** token blast radius · **B** false green · **C** inheritance leak · **D** split doctrine · **E** declared, not built. Seventeen of eighteen nodes are class **D** — that is the map's whole thesis. **N-012** is the one class **A**                                                                              |
...
| N-019 | Six shipped homes still contradict the `_tasks` fact their own fix commit corrected          | task     | D     | none                                          | A     | no                |
...
| **A** | N-002 · N-007 · N-016 · N-017 · N-019 | **Shared shape** — a restatement that drifted from its owner, each with the correct copy already in the tree to copy from. No trade-off: a replacement, a deletion, a re-wording. **N-019 joins it 23/08/2026** and is the purest case: `111637a` wrote the canonical form three times in the same commit that left six copies wrong | **now**  |
...
- **N-018** · `how-to/CONTEXT.md:22` calls `CLI-TOOLING.md` the _"CLI reference for **every dev
  ...
  together either way.
- **N-019** · **Charted 23/08/2026 at `111637a`, against a fix for this exact fact that landed
  mid-pass.** `_tasks` do **not** _"run on `copy` and never on `update`"_ — Copier runs them on
  both, and the seed-once property comes from the explicit `when: "<% _copier_operation == 'copy' %>"`
  gate. That is why the same commit raised `_min_copier_version` **`9.0.0` → `9.6.0`**:
  `_copier_operation` first shipped in Copier 9.6.0, and on anything older the expression is
  undefined rather than false, so the gate does not hold open — **it holds shut**, and a fresh
  generation silently skips seeding `README.md`, the four version files, the scale-planning map and
  the three blank accumulators. `111637a` states the correct account four times — `copier.yml:57-58`
  and `:842-844`, `06-GENERATION.md:140-141`, `14-UPDATING.md:33-35` — and its own `CHANGELOG.md`
  claims _"the seven places that had it backwards"_. **Six live statements still contradict it, and
  every one of the six ships:**
  1. `how-to/src/TEMPLATE-GUIDE/06-GENERATION.md:22` — _"`copier update` runs a different tail: **no
     `_tasks` at all**"_ (`f5fef31`, untouched by the fix)
  2. `06-GENERATION.md:188` — _"Tasks 1, **2** and 4 are guarded by `when: _copier_operation ==
'copy'`"_. **It was true at `9122ca5` and `111637a` falsified it**, by ungating the
     manifest-branding task so it runs ahead of `uv lock` — **48 lines below the same commit's own
     correction** at `:140`
  3. `06-GENERATION.md:209` — _"`_tasks` do not run on an update to clear it"_. `111637a`
     **rewrote that migration-table row** and carried the clause through, 69 lines below `:140`
  4. `how-to/src/TEMPLATE-GUIDE/15-TROUBLESHOOTING.md:96` — _"`_tasks` run on `copy` only"_ (`f5fef31`)
  5. `how-to/src/TEMPLATE-TOKENS.md:421` — _"the cost, which is that `copier update` never runs
     `_tasks`"_ (`7cd385d`) — written **about the very task `111637a` ungated**, so the cost it
     names no longer exists
  6. `project-management/docs/VERSIONING-GUIDE.md:265` — _"Those tasks run on `copier copy` and
     never on `copier update`"_ (`263c6af`), with `:269` building a corollary on it

  **Four are false as written; two — `06-GENERATION.md:209` and `15-TROUBLESHOOTING.md:96` — are
  true local conclusions resting on the false premise.** The distinction changes the wording of the
  fix, never the cause. **Three of the six sit in the one file the fix pass edited**, and one of the
  three was carried through by that pass rewriting its line. **Nothing could have enumerated them, on
  two independent grounds.** `doctrine-drift.sh:61-67` scans exactly five trees — `code/docs`,
  `.claude/skills`, `code/workflows`, `project-management/workflows`, `how-to/workflows` — so
  `how-to/src/`, `project-management/docs/` and `copier.yml` are **all outside its scope**; and
  `:175-182` reads **fenced code only**, defended at `:31` as _"Prose is free; examples are the
  contract"_. All six homes are prose, in trees the only doctrine gate does not open. This is the **fourth conditional row N-004
  owes**, and the only one of the four needing no prior spelling decision: `111637a` already wrote
  the canonical form, four times, in the same commit.
...
- **Which sections of a shipped guide a project may edit, where the guide answers in prose.**
  ...
  `:158`. **Neither is false and both yield the same action** — all 7 `_TBD` markers sit in §3 — so
...
**Node-count invariant: 18 open + 1 resolved = 19 = N-019** — **extended at `111637a`**; the sixth
pass refuted four of its five candidate findings, moved one to fog, and charted **N-019**, which
joins Batch A. Per batch: A 5 · B 4 · C 2 · D 2 ·
E 3 · F 2. Per class: **D** 17 · **A** 1.
