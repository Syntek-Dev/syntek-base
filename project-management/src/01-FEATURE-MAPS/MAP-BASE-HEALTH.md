...
**Status**: **Resolved 23/08/2026 — frontier 0, fog of war 0, 60 of 60 nodes settled.** One
Destination condition is outstanding and is not a node: the root `GAPS.md` must carry standing
limitations only, and it holds one dated task — the `main` fast-forward, which retires when this
branch's pull request merges. Nothing further is scheduled against this map.
...
**Frontier open**: 0 · **Blocking open**: 0 · **Resolved**: 60 — recounted from the tables
23/08/2026 after **sitting 8** settled **N-021** and **N-026** and discharged the whole of
_Fog of war_: **0 + 60 = 60 = N-060**. Per batch: A 0 · B 0 · C 0 · D 0 · E 0 · unbatched 0.

**THE FRONTIER IS EMPTY AND SO IS FOG OF WAR.** Every one of the sixty nodes this map charted is
settled, and the eleven live fog items are dispositioned — seven refused or discharged, two
resolved in the sitting rather than graduated, one routed to the map that owns it, one already
struck. **One Destination condition remains unmet and it is not a node**: the root `GAPS.md`
must carry standing limitations only, and it carries one dated task — the `main` fast-forward,
which retires when this branch's pull request merges. The map cannot close that itself.

**Sitting 8's thesis is that the two nodes left parked were parked correctly, and executing one
of them found a class nobody had charted.** N-021's blocker held on all three rows at HEAD and
became **SL-3** — the second node closed as _accepted_ after N-020, on the same reasoning one
level in: three of the four runbooks have no subject, and the node's own gate was rehearsal.
N-026 was **executed rather than reviewed**, on N-010's lesson, and the first run of the
corrected scaffold produced **11 `docs-pairing.sh` violations of which seven were pre-existing**
— the project's own scaffolder has been emitting documentation that fails the project's own gate
since that gate existed, unseen because no app has been scaffolded since.

**And the sitting opened by repairing its predecessor.** Sitting 6 deleted a fog-of-war item
outright — its insertion replaced that item's heading and never re-appended it — so the map lost
an open question inside the commit closing N-060, with nothing to catch it: no gate reads that
section and the header count excludes fog. Restored here. **Twice in three sittings a settlement
has produced an instance of something it was not looking for**, which is the strongest argument
this map has made for the doubled-leg review shape.

**Previously 2 · 0 · 58**, recounted 23/08/2026 after **sitting 7** settled **N-023**,
**N-045**, **N-049** and **N-039**:
**2 + 58 = 60 = N-060**. Per batch: A 0 · B 0 · D **0** · E **2** · unbatched **0**.
**Batch D closes with N-049, and every remaining node is one the map had already named a
disposition rather than a sitting** — N-021 and N-026 (both parked on unmeasured premises) and
N-039, now open on its gate half alone. **The Destination is three decisions away**, and none of
the three is a sweep.
**Sitting 7's thesis is that a node can be wrong about which of its two homes is the unswept
one.** N-049 read the router as the site that had missed a decision; measured, **54 of 118
tracked `*.sh` carried the practice the owner's absolute forbade**, 20 of them outside the tree
the router exempted, so neither text described the repository. It settled at neither pole —
**route, never restate** — and the owner turned out to contradict _itself_ as well, binding every
`*.sh` in one paragraph and only `code/src/` in the bullet that enforced it. **The half that
stayed banned had never been measured**, and was not clean: two application-source files cited a
guide, and both are repaired by stating the reason rather than pointing at it.

**Previously 6 · 0 · 54**, recounted 23/08/2026 after **sitting 6** settled **N-060**:
**6 + 54 = 60 = N-060**. Per batch: A 0 · B **0** · D 1 · E 4 · unbatched 1.
**Batch B's frontier is empty, and the fog-of-war taxonomy entry's own trigger has therefore
fired** — it says _"settle it once B closes, not before"_. Nobody is scheduled against it, which
is now true of two fog items rather than one; recorded here so the next reader does not have to
notice it from the batch tables.
**Sitting 6's thesis is that a gate which had written its own absence down still needed the
population measured before the clause could ship.** `FORWARD-VOICE.md` Section 5 had stated the
gap in terms since 20/08 and nothing read it; the settling pass found **83 undeclared sites
across 45 files** behind that sentence, marked 75 lines green first, and only then added the
check — N-025's warning honoured for the fourth time on this map. **Seven of the 83 were not
marker candidates**: six guides citing bare `SECURITY.md` under `code/docs/` were resolving
sibling-first to the excluded ROOT file and meant the OWASP guide, so the node's own sweep
repaired a class nobody had charted. **The node's "12 marked sites" had doubled to 24 in
two days**, unremarked, which is the fourth charted number on this map to age inside 48 hours.

**Previously 7 · 0 · 53**, recounted 22/08/2026 after **sitting 5** settled **N-056** and
**N-058**: **7 + 53 = 60 = N-060**. Per batch: A 0 · B **1** · D 1 · E 4 · unbatched 1.
...
> **Two of the three conditions were met on 23/08/2026** — the frontier emptied at sitting 8 and
> fog of war was discharged in the same pass. The third is one entry away and the map cannot
> close it from here: `GAPS.md` holds the `main` fast-forward, a dated task with a named owner
> that retires when this branch merges. **A map that closed itself by deleting that entry would
> be the register defect this map exists to catch**, so it is left standing and named.
...
| `MAP-CLAUDE-DESIGN-HANDOFF`   | Charting, 23 open, **7 blocking**  | **Found 23/08/2026 by sitting 6's sibling read — the ninth map, and this table's fifth consecutive failure of a different kind: not a map never listed, nor one described wrongly, nor one that did not exist yet, but one that existed and was never read into this table at all.** Routes nothing here. **It shares four files with sitting 6 and collides on none of them, measured by name rather than asserted**: its N-011 claims `.claude/CLAUDE.md`'s `## Design system` block where sitting 6 marked Section 9's register paragraph; its two `CONTEXT.md` mentions are `01-FEATURE-MAPS/CONTEXT.md`, not the root file sitting 6 marked; and its numbering rule at `project-management/workflows/CLAUDE.md:67` sits **one line above** the citation sitting 6 marked at `:66` — near enough that whoever opens either owes a re-measurement rather than these numbers. **One inbound correction, recorded and not fixed here:** that map asserts `GAPS.md` and `DEFERRED.md` _"both hold no entries — each is a shipped skeleton"_, which sitting 5 falsified on both halves — the register is excluded and seeded now, and this repository's copy carries real entries. It is that map's line to correct |

> **The duty has found something on every one of its five executions — 16/08, 18/08, and
> sittings 1, 2 and 6.** Five of five, on a table this map maintains about itself, and it is the
> argument for leaving the gate item unticked rather than a reason to distrust any one session.
> **Nine live sibling maps, not seven** — `MAP-CLAUDE-DESIGN-HANDOFF` was added 23/08/2026.
> **Seven live sibling maps, not five** — the 16/08–18/08 reading, kept as written. Nothing new was routed here on 18/08: `MAP-ABSENCE`'s eight remain
...
| N-021 | Write the four security incident runbooks — each performed once against non-production first                         | task            | 23/08/2026 | **Sitting 8 — closed as _accepted_, the second node on this map to take that outcome after N-020, and for the same reason one level in.** Re-measured at HEAD: `config/settings/base.py` declares a single `default` alias so there is no `admin_db`; the string `audit` appears in no `.py` under `code/src/django/`; `apps.core.encryption` does not exist and `cryptography` is declared but imported nowhere. **Three of the four have nothing to be performed against**, and the node's own gate was that each is rehearsed against non-production BEFORE it is written — so writing them would fail the node's criterion in the same act as satisfying its title. **The absent three are things a generated project builds**, not things this template withholds, so a runbook here would carry the concrete shell commands for revoking a token in a connection that does not exist: Batch E by its own definition. **The fourth was deliberately not split off** — Valkey has a subject and its runbook is writable, but `MONITORING-AND-INCIDENT.md` names the four in one paragraph as one gap, and shipping one of four leaves that paragraph three-quarters true, which reads worse than honestly none. Became **SL-3** in `GAPS.md`, with a per-row reopen trigger: each reopens when an application layer gives it a subject, which is the only order in which it can be rehearsed                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| N-026 | Does the scaffold emit the per-app `api.py` and `services/` the conventions assert?                                  | grilling        | 23/08/2026 | **Sitting 8 — settled at `services/` yes, `api.py` no, and executing the scaffold found seven defects nobody had charted.** The decision: `new-django-app.sh` now emits the layout `apps/CONTEXT.md` names — `services/`, `schemas/` and `tests/` beside the existing `models/`, each with its documentation pair — because the script already wrote an app `CLAUDE.md` telling the developer to put business logic in `services`, a directory it did not create. **`api.py` is refused**: that convention binds an app with an HTTP surface, `apps.core` and `apps.health` have both declined it correctly, and an empty router in an app with no endpoints is a file the convention would then have to defend. `NINJA-CONVENTIONS.md` is qualified from _"every module exports an `api.py`"_ to the module that serves HTTP. **The node was executed, not reviewed** — N-010's lesson — against the live dev stack: the first run produced **11 `docs-pairing.sh` violations, seven of them in blocks the script has emitted since before that gate existed** (a banned `## Rules` heading in two CONTEXT.md files, a `migrations/CONTEXT.md` with no tree, four undescribed tree rows). **The project's own scaffold has been emitting documentation that fails the project's own gate**, unseen because no app has been scaffolded since — `apps/health` was hand-built by N-035. All eleven fixed, then re-executed: 19 files, docs-pairing green, docs-length green, every package importing                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| N-039 | Eight shipped guides carry leaked tool-call artefacts, and have since 01/08/2026                                     | grilling        | 23/08/2026 | **Sitting 7 — the residue had half-cleared itself, and the home named for the gate could not have hosted it.** Charted at ten sites across eight files; measured at **four across three**, `ea78457` having removed the rest incidentally while fixing unrelated guides and recorded nothing. The four are deleted and a tree-wide sweep returns nothing, so **the rule was true before the instrument shipped** — N-025's precondition, met by measurement rather than by assertion. **The instruction to put the clause in `copy-slop.sh` was refused on a fact**: that script's population is `apps/marketing/pagedata/*.py` and `templates/marketing/*.html`, so it reads **zero files** in this template and could never reach `code/docs/` — a clause there would have been a check over a population it cannot see, which is this batch's own defect. **It went to `conflict-markers.sh` instead**, whose 1,291 tracked text files are the right population and whose subject is the same shape: _a literal string that must never survive into a tracked file, in any file type_ — the argument its own header already makes for being a separate audit. **The name is now narrower than the contents, deliberately**: renaming was refused because `Unresolved conflict-marker audit` is one of the three job names the queued branch-protection flip adds, and the string appears in 19 files — a rename mid-flip risks a required check that never arrives, which this map has already charted once as N-057. The pattern is **assembled from parts** in `_lib/`, or the gate would redden on its own source, and both detectors share one `awk` because two detectors for one class that disagree is how the conflict-marker defect survived two releases. `--self-test` gains four probes: fires on three tag forms, silent on ordinary markup (`<div>`, `<content-type>`), honours the shared directive, and **neither class matches the other's**. Two legitimate carriers found and marked — this map and `MAP-ABSENCE`, each quoting the tag in order to describe the defect |
| N-060 | Nothing reads `copier.yml`'s `_exclude` and compares it against citations, so Direction B is declared and unenforced | task            | 23/08/2026 | **Sitting 6 — the rule was made true before the instrument shipped, and the sweep found seven miscitations nobody had charted.** `doc-references.sh` gains **Check 3**: it parses `copier.yml`'s `_exclude`, subtracts the 24 negations, the nine `_tasks` seeds and the regenerated `uv.lock`, and fires when a **shipping** file cites one of the **40** paths left without `doc-references: template-only` or `ignore`. **Population measured first, at 83 sites across 45 files** — 56 of them `copier.yml` — against 24 already marked; 75 lines were marked and the tree was green before the clause was added, on N-025's standing warning. **Seven of the 83 were not marker candidates at all**: a bare `SECURITY.md` under `code/docs/` resolves sibling-first to the excluded ROOT file, so six guides citing the OWASP guide were pointing at the licence-policy file by accident and are repaired to `code/docs/SECURITY.md`; the seventh (`rust/SUPPLY-CHAIN.md:59`) means an upstream crate's own file and takes `ignore`. **Three classes are deliberately outside the set** and each is asserted directly by `--self-test`: surface-gated (`INCLUDE_RUST` and its siblings — those paths ship wherever the surface was taken, so the marker would be false there), excluded-and-seeded, and re-included pair files. **The check is keyed on the two markers alone, never on `is_naming_row`** — `e.g.` says a citation is illustrative, not that it survives generation. Proven in both directions: two new fixtures whose every path RESOLVES here, so only Check 3 can fire; four direct assertions on the derived set; and a live negative control on `CONTEXT.md:129` (marker off → exit 1, marker on → exit 0). **Cost measured, not assumed**: a basename index makes the clause ~5s on a 1m41 baseline, against 33s without it. **`check-template-tokens.sh` caught the author writing a live `<:` opener into a rendered file** — Batch A's own class, inside the commit closing a Batch B node                                                                      |
| N-023 | Two accepted Rust advisories have no re-check mechanism — rescoped to the residue sweep they left behind             | task            | 23/08/2026 | **Sitting 7 — the node was already dead and its residue was exactly the size the map recorded.** `deny.toml`'s `ignore` list has been empty since `b805774` (an MSRV bump to 1.92 let the resolver take zbus 5.19, which no longer pulls `quick-xml`), so both charted trigger arms had no subject. **The 9 sites across 7 files re-measured exactly** — the only charted site that had died is `copier.yml`, whose text now says the shared files carry Slint's licence exceptions _unconditionally_ and asserts nothing about accepted advisories. **What was swept is the assertion, not the doctrine**: every site keeps _an advisory reached only through the AT-SPI stack is never answered by dropping accessibility_, which is still true and still load-bearing, and stops claiming two suppressions are live. Four cross-reference labels retargeted from _"why two AccessKit advisories are accepted"_ to what `SUPPLY-CHAIN.md` actually owns. **The tenth site the map named separately was real and is fixed with them**: `stack-rust/SKILL.md` still called `rust-version` _"the MSRV our source needs"_, the framing `SUPPLY-CHAIN.md` refuted on 16/08 — it is a **resolution input** under `resolver = "3"`, which is why a floor left behind the channel holds the whole graph back                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| N-045 | The Bruno request template a developer copies documents a GraphQL API this project does not have                     | task            | 23/08/2026 | **Sitting 7 — rewritten for the stack this project actually serves, and the twin fossil went with it.** The template was GraphQL end to end: a `/graphql/` POST, `body:graphql` blocks, four folders that do not exist, an undefined `{{some_variable}}`, and the advice that _"GraphQL always returns 200 even on errors"_ — which teaches the one habit REST testing must not have. Replaced by the house form read off the live `health/` requests: a real contracted status code, the unwrapped `OutSchema`, the `X-Request-ID` header, and the error envelope `code/docs/api-design/AUTH-AND-ERRORS.md` owns, with the failure path shown as a test rather than an afterthought. **The collection's own `CONTEXT.md` was already correct** and is not touched — this was one wrong file, not a wrong convention. **The twin fossil is the sharper half.** `tests/api.sh` and `test-api.yml` both justified `--exclude-tags manual,wip` with an origin project's request set — seeded SEO records, a Cloudinary mock, a blog post and a `user(id)` **query**, GraphQL vocabulary in a template that ships neither — while the collection carries **no `tags` block at all**, so the flag excludes nothing and a green run is green over everything that exists. Both now say so, because a reader inferring "something was skipped" from a flag is the false green this map is named for. **And the workflow cited `BUG-API-SUITE-UNRUN-09-06-2026.md`** — a per-project instance artefact named in a shipped file, invisible to `doc-references.sh` because it sits in an unbackticked comment. Deleted; the class is sitting 2's, found here by reading rather than by a gate                                                                                                                                                                                                                                                                                                                                                                                                           |
| N-049 | N-016's comment rule contradicts itself across its two homes — a resolved node, live again                           | grilling        | 23/08/2026 | **Sitting 7 — re-typed `task` to `grilling` on measurement, and the node had it backwards.** It read the router as _"not a rival view, an unswept site"_. Measured: **54 of 118 tracked `*.sh` carry a comment naming a `.md` path**, including every audit header and the pre-PR hook — and **20 of those sit outside `code/src/scripts/`**, so even the router's blanket dev-scripts exemption would not have covered them. Neither home described what the repository does, and the owner's absolute (_"nothing in one points outside the file"_) condemned the enforcement layer wholesale. **Settled at neither pole: route, never restate.** A file whose **job is to enforce a documented rule** may **name** the document that owns it — because the alternative, _state the reason in full here_, puts a second copy of the rule inside its own enforcement, which is Batch D by construction. Restating the rule's content stays banned, and the permission does not travel: a service module implements a feature rather than enforcing a guide, so there a pointer is still a substitute for a reason. **The owner also contradicted itself and nobody had noticed**: its Scope paragraph bound every `*.sh` _"wherever in the repository it sits"_ while the bullet enforcing it said _"anything under `code/src/`"_ — narrower than its own scope, leaving 20 files addressed by a rule that did not reach them. **The still-banned half was measured for the first time and was not clean**: two application-source files cited a guide — `config/settings/base.py` and `tests/e2e/conftest.py` — neither of which enforces anything. Both repaired by stating the reason in full, which is what the rule asks for                                                                                                                                                                                                                                                                                                                                                             |
...
**Frontier empty — N-060 settled 23/08/2026, sitting 6, see _Resolved decisions_.** The class is
**not** called closed, on Batch A's precedent and on its own record: this is the class whose
members are hardest to name in advance, because a false green announces nothing. What is
different about this emptying is that the last node was a gate that had **written its own
absence down** — `FORWARD-VOICE.md` Section 5 stated the gap in terms and nothing read it — so
the class's final member was documented and unenforced rather than unnoticed.

**And the batch produced one more member on its way out, inside its own closing commit.** The
author wrote a live `<:` block opener into `doc-references.sh` while describing the class of
`_exclude` entry the new check skips; `check-template-tokens.sh` caught it before it committed.
That is a **Batch A** defect (a delimiter reaching a parser) produced by a **Batch B** repair,
and it is the second time on this map that settling a node created an instance of another class
— N-035 was the first. Recorded because the pattern now has two data points and neither was
predicted by the node that produced it.
...
out to be narrower and sharper than charted. The batch left **N-060 alone in Batch B**, and
sitting 6 took it.
...
**N-060 — SETTLED 23/08/2026, sitting 6** (see _Resolved decisions_). The charted block is left
as written, with two corrections measured at HEAD:

- **"the 12 marked sites" is 24.** The count was taken on 21/08 and the 22/08 work doubled the
  adoption before anyone looked again. The node's own worked answer was therefore twice the size
  it recorded — which did not change the remedy, and is the fourth time on this map that a
  charted number aged inside two days. **Carry the command**: the population regenerates from
  `copier.yml`'s `_exclude` and a `git grep` for the two markers, and the settling session
  scripted it rather than counting by hand.
- **"the decision is already taken" held, and it was not the whole decision.** N-031 settled
  which citations are the deliberate-dangle class; it never asked what a **surface-gated**
  exclusion is. That question had to be answered before a line of the check could be written,
  and answering it wrong would have demanded `template-only` on every `RUST.md` and `MOBILE.md`
  citation — false in every project that took the surface. The `task` type survived because the
  answer was already implicit in the repository (those index rows carry a `rust-only` flag
  today), but the node did not know it was carrying the question.

Charted below. **N-060 came out of N-031's settlement, and `FORWARD-VOICE.md` Section 5 had
already charted it in prose.** That section states the gap in terms — _"Nothing reads `copier.yml`'s
...
**Frontier empty — N-049 settled 23/08/2026, sitting 7, see _Resolved decisions_.** The class is
**not** called closed, on this map's standing practice: Batch D has been declared closed twice
before and reopened both times. What it demonstrated on the way out is worth keeping — **a split
doctrine can have both homes wrong**, and the node that charts it can be wrong about which one is
the unswept site. N-049 named the router; the measurement convicted the owner as well, and the
owner turned out to contradict itself inside one section.
...
**Frontier empty — N-021 and N-026 settled 23/08/2026, sitting 8, see _Resolved decisions_.**
The class closes **last of the five**, which is the right order for it: "declared, not built" is
the only class whose members are found by asking what a document promises, and the two that
survived to the end were the two whose promises could not be kept here at all. N-021 became a
standing limitation; N-026 kept half its convention and refused the other half, and executing
the refusal found seven pre-existing defects in the scaffolder's own output.
...
**Frontier empty — N-039 settled 23/08/2026, sitting 7, see _Resolved decisions_.** The
unbatched residue is gone, and with it the argument its own intro kept making: _"four unbatched
nodes against five classes is no longer a residue"_. It never reached five. What the class
leaves behind is one measured fact for the taxonomy question — **the sixth class it declined to
assert was real**, and it turned out to belong inside an existing audit rather than beside it,
because _a literal string that must never survive into a tracked file_ already had a home.
...
**N-039 — SETTLED 23/08/2026, sitting 7. Both halves closed in one sitting, the second after a
refusal.**
The residue is **gone**: measured at HEAD it had already shrunk from the charted ten sites across
eight files to **four across three** — `ea78457` removed the rest incidentally while fixing
unrelated guides, so more than half the node had been discharged by a session that was not
looking for it and recorded nothing. The four survivors
(`api-design/NINJA-CONVENTIONS.md:334-335`, `rendering/CLAUDE.md:41`, `rendering/CONTEXT.md:19`)
are deleted, and a repository-wide sweep returns nothing. **The rule is now true, which is the
precondition N-025 set for building the instrument.**

**What is open is the home, and the obvious answer was measured and refused.** The sitting was
directed to add a clause to an existing audit rather than write a new one, and named
`copy-slop.sh` for it — **wrongly, on a fact**: that script's population is
`apps/marketing/pagedata/*.py` and `templates/marketing/*.html`, so it reads **zero files** in
this template and could never see `code/docs/`. A clause there would be a check over a population
it cannot reach, which is the Batch B defect exactly.

- **`conflict-markers.sh` is the right population and the wrong name.** It scans **1,291 tracked
  text files** across every file type, and its subject is precisely this shape — _a literal
  string that must never survive into a tracked file_. Its own header argues it is a separate
  audit **because** it spans every file type rather than one language, which is equally true of
  transcript residue.
- **Renaming it is not free, and the cost is live.** `Unresolved conflict-marker audit` is one of
  the **three job names the queued branch-protection flip adds** (20 → 22 contexts, applied after
  this branch merges), and the string appears in 19 files including `lefthook.yml`,
  `shipped-readme.sh`'s register and `template-update.sh`. Renaming mid-flip is the one change
  this map should not make on its own initiative.
- **Settled: widen `conflict-markers.sh` in place**, over the three-way choice of renaming it
  (and re-pointing a queued flip) or adding a twenty-third required context for one grep.
  **Re-typed `grilling`** — the node was `task` on the premise that only a deletion was owed,
  and the deletion turned out to be the smaller half.
- **It is also the first node to reach the taxonomy question with the trigger already fired**
  (see _Fog of war_), and it answers one instance of it: a one-line grep does **not** earn its
  own audit, because the class it belongs to already had one. That is evidence for the taxonomy
  settling, not a substitute for it.

**The charted block is left as written below.**

**N-039 — machine-authored residue in shipped prose, two weeks old and past ~~thirteen~~ 49
releases.**
Ten occurrences of `</content>` and `</invoke>` across **eight** files, all of which **ship** <!-- conflict-markers: ignore -->
...
> **DISCHARGED 23/08/2026, sitting 8 — nothing below is open.** Every item is dispositioned in
> the table, and the narrative under it is kept **as written**: an entry's working is the
> evidence for its disposition, and deleting it would leave a verdict with nothing behind it.
> Read the table for the state; read the bullets for why.
>
> **Two were resolved in the sitting rather than graduated to nodes**, on Sam's call — the map
> is being closed, and charting a node on a map whose frontier is empty would reopen it to
> record work that took twenty minutes.

| #   | Item                                                   | Disposition                                                                                                                                                                                                                                                                                                                |
| --- | ------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | The five classes as a **shipped taxonomy**             | **Refused.** All five have paid out and the trigger it set for itself fired when Batch B emptied; N-039 then answered its one live instance — a grep does not earn an audit when its class already has a home. Shipping the taxonomy is a new epic, not this map's item                                                    |
| 2   | `is_exempt()` vs the derived ships-predicate           | **Routed.** It is `MAP-RULE-OWNERSHIP` N-009, on that map's frontier, and sitting 6 built the predicate it needs. Recorded on both maps                                                                                                                                                                                    |
| 3   | Cited **sections**, not paths                          | **Refused, on a built and measured remedy.** A first pass returned 3 resolving and **9 false positives**, every one a routing table describing a guide rather than citing a heading; the arrow form's right-hand side is not cleanly delimited either. A citation of a section and a description of one are the same words |
| 4   | "A dispatched finding is evidence about where to look" | **Discharged** — a lesson, already stated, with no decision left in it                                                                                                                                                                                                                                                     |
| 5   | The `:3080` port residue                               | **RESOLVED IN THE SITTING**, and the item had it backwards — see below                                                                                                                                                                                                                                                     |
| 6   | A wrong `code/src/django/` citation                    | **Accepted.** The blanket skip is deliberate and carries its rationale in the script; it can only bite once a project has code, and separating a misspelling from a legitimately-absent path needs the import graph                                                                                                        |
| 7   | Do routed findings need a mechanism?                   | **Refused.** Six sibling reads have now caught six **different** failures, so the duty works; a shared inbox is a wayfinder-level design and not this map's to build                                                                                                                                                       |
| 8   | ~~`docs-pairing.sh` on a directory with neither~~      | Discharged 15/08/2026 by N-017 itself                                                                                                                                                                                                                                                                                      |
| 9   | Expiry-gate generalisation                             | **Refused on its own stated threshold** — it set "two registers with the same shape" and has one, `deny.toml`'s instance having been deleted and `docs-length-allow` being immune by format                                                                                                                                |
| 10  | The ratchet beyond line count                          | **RESOLVED IN THE SITTING** — refused as a mechanism, written as a rule in both owning guides. See below                                                                                                                                                                                                                   |
| 11  | Line numbers in verdict blocks                         | **Discharged** — settled in practice rather than by decision: every sitting since 4 has re-located by string before editing, and sittings 4, 5 and 7 each caught drift that way                                                                                                                                            |

**Item 5 resolved, and the entry's own framing was wrong.** It read as _"`47bafb1`'s port sweep
missed two files"_ — implying the two `:3080` sites were stale. Measured 23/08/2026: the
population is **five sites across five files**, and **every one of them is correct**. The
authority is `code/src/docker/docker-compose.usXXX.dev.yml.example:31`, which publishes
`127.0.0.NNN:3080:80` (the test twin publishes `3081`), so the sweep was right to leave them.
**What is wrong is the guide the others say they are summarising** — `how-to/docs/GIT-WORKTREES.md`
gave the dev URL **no port at all** and the test URL `:81`, which is the _main_ stack's port on a
different address. **And the defect was not only in prose:** `development/server.sh:159` printed
the worktree URL portless too, so the one place `.claude/CLAUDE.md` Section 7 says a URL may be
read from rather than remembered was printing an **unreachable address**. Script and guide both
corrected; the five correct sites were left alone.

**Item 10 resolved as a refusal with a written rule.** A ratchet needs a measured per-change
number and a baseline to diff against, which is exactly what `docs-length.sh --since` has and
what neither of the other two budgets does — "how long a contributor will wait for the required
set" and "what the skill roster costs a model" are not per-change measurements. So no second
mechanism was built. What was missing was cheaper and is now written where each budget lives:
`git/PR-AND-REQUIRED-CHECKS.md` says **state the size the set will be, not only the name being
added**, and `skill-authoring/CRAFT.md` says **when you lengthen a description, say what the
roster now costs**. Both name the shared shape rather than restating the line-count rule.
...
- Whether the five defect classes above should become a **shipped taxonomy** that audits are
  ...
  **23/08/2026 — the trigger this entry set for itself has fired, and nobody is scheduled
  against it.** The rule was _"settle it once B closes, not before"_; **Batch B's frontier is
  empty** as of sitting 6. What the class finally yielded is worth carrying into the settling:
  its last member was a gate whose absence was **already written down in a shipped guide**
  (`FORWARD-VOICE.md` Section 5) and read by nobody for three days — so "a false green announces
  nothing" has a sharper form, **a declared gap announces nothing either**. Two candidate classes
  are still on the table from 16/08 (_built, not declared_; _transcript residue_) and neither is
  asserted here. **Two fog items now have a fired trigger and no owner** — this and the
  taxonomy's sibling below — which is itself the argument that fog of war needs a scheduling
  mechanism rather than a fifth paragraph.

- **`doc-references.sh` now derives what ships, and `is_exempt()` still hardcodes it — two
  answers to one question, in one file.** Recorded 23/08/2026 by sitting 6 and deliberately not
  settled. Check 3's `is_template_only()` computes the ships / does-not-ship predicate from
  `copier.yml` and is therefore right by construction; `is_exempt()` three screens above it
  still says _"none of these ship"_ of `research/*`, `learning/*` and
  `project-management/src/01-FEATURE-MAPS/*` while **nine files inside those trees do** — the
  re-included documentation pairs and `MAP-000-TEMPLATE.md`. That is
  **`MAP-RULE-OWNERSHIP` N-009**, measured by its own N-011 and confirmed here at HEAD.
  **It stays on that map, on Sam's call**, and the reason is the standing one: settling another
  map's `grilling` node in passing is how a node's row comes to claim a fix nobody reviewed.
  What sitting 6 leaves behind for it is the **predicate itself** — N-009 no longer has to
  design one, only to decide whether `is_exempt()` should call it. Recorded on both maps.

- **Whether anything can check what a skill claims about the tree.** Surfaced by N-037, and larger
  than it.
  **This bullet's heading was deleted by sitting 6 and restored by sitting 8, three hours apart.**
  The bullet above was inserted by replacing this one's first line and never re-appending it, so
  two fog items ran together into one and the map lost an open question inside the commit that
  closed N-060. Nothing caught it — no gate reads this section, and the count in the header does
  not include fog. Recorded rather than quietly repaired, because it is this map's own defect
  class committed by the session that was clearing it, and the second time in three sittings that
  a settlement produced an instance of something it was not looking for. `skill-conformance.sh` checks a skill's **shape** — spec fields, the field set, fork
...
| ---------- | ----------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------- |
| 23/08/2026 | N-021 · N-026                 | **Sitting 8 — the map closes: frontier 0, fog of war 0, sixty of sixty settled.** It opened by repairing sitting 6, which had **deleted a fog-of-war item** — an insertion replaced that item's heading and never re-appended it, so an open question vanished inside the commit closing N-060 and nothing caught it: no gate reads that section and the header count excludes fog. **Twice in three sittings a settlement produced an instance of something it was not looking for** (sitting 6 also wrote a live `<:` opener into a rendered file), which is this map's strongest measured argument for the doubled-leg shape. **N-021 held on all three rows** — no `admin_db` alias, no audit model, `apps.core.encryption` absent and `cryptography` imported nowhere — and became **SL-3**, the second node closed as _accepted_ after N-020. The fourth runbook has a subject and was deliberately **not** split off: `MONITORING-AND-INCIDENT.md` names the four as one gap, and three quarters of a paragraph reads worse than none. **N-026 was executed rather than reviewed**, against the live dev stack, and that is what earned the sitting: the corrected scaffold's first run raised **11 docs-pairing violations, seven of them pre-existing** — a banned `Rules` heading in two emitted CONTEXT.md files, a `migrations/CONTEXT.md` with no tree, four undescribed tree rows. **The project's own scaffolder had been emitting documentation that fails the project's own gate for as long as the gate has existed**, unseen because no app has been scaffolded since `apps/health` was hand-built. Settled `services/`+`schemas/`+`tests/` yes, `api.py` no; re-run clean at 19 files. **Fog of war discharged in one pass**: seven refused or discharged, one routed to `MAP-RULE-OWNERSHIP`, one already struck, and **two resolved in the sitting rather than graduated** — the `:3080` residue, whose entry had it backwards (all five sites were correct; the guide summarising them was not, and `server.sh` printed an **unreachable** worktree URL), and the ratchet question, refused as a mechanism and written as a rule in both owning guides. **Charted nothing.** Counts recounted from the tables: **0 + 60 = 60 = N-060**, every batch 0                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | [x]              |
| 23/08/2026 | N-023 · N-045 · N-049 · N-039 | **Sitting 7 — three nodes that shared only their type, and the one typed `task` was the one that needed grilling.** Every premise re-measured before a question was asked, and **two of the three had moved**. **N-039 had already half-discharged itself**: charted at ten sites across eight files, measured at **four across three** — `ea78457` removed the rest incidentally, recording nothing, which is the second time on this map a node shrank between sittings without anyone noticing. **N-023 re-measured exactly** at 9 sites / 7 files, its one dead site being `copier.yml`; the sweep kept the doctrine (an advisory reached through the AT-SPI stack is not answered by dropping accessibility) and deleted the claim that two suppressions are live, and took the tenth site the map had named separately — `stack-rust` still calling `rust-version` a promise about our source where it is a **resolution input**. **N-045 held exactly** and its twin fossil was the sharper half: two files justified `--exclude-tags manual,wip` with an origin project's request set, in GraphQL vocabulary, against a collection carrying **no tags at all** — a flag that excludes nothing, described as though it excluded something. A shipped workflow also cited `BUG-API-SUITE-UNRUN-09-06-2026.md`, a per-project instance invisible to the gate because it sat in an unbackticked comment. **N-049 is the sitting's finding.** Charted `task` on "one home missed the decision", it was re-typed `grilling` by measurement: **54 of 118 tracked `*.sh` carry a comment naming a `.md` path**, 20 of them outside the tree the router exempted, so **both** homes were wrong and the owner's absolute condemned the entire enforcement layer including its own audit headers. Settled at neither pole — **route, never restate**, discriminated on whether the file's job is to enforce the named document. **The owner contradicted itself too**, binding every `*.sh` in its Scope paragraph and only `code/src/` in the bullet enforcing it. **The still-banned half had never been measured and was not clean**: `config/settings/base.py` and `tests/e2e/conftest.py` both cited a guide, and neither enforces one. **Batch D closes.** **One instruction was refused on a fact**: the sitting was directed to put N-039's gate clause in `copy-slop.sh`, which reads **zero files** here — its population is marketing copy. The right population is `conflict-markers.sh`'s 1,291 text files, and renaming that audit collides with the queued ruleset flip, so it went to `conflict-markers.sh` instead — 1,291 text files, the same shape, and the argument that script's own header already makes for being a separate audit. **Its name is now narrower than its contents on purpose**: renaming it would re-point one of the three job names the queued ruleset flip adds, which is the N-057 failure bought for a tidier label. **Batch D closes and the unbatched class empties**, leaving a frontier of two nodes in one batch. **Charted nothing.** Counts recounted from the tables: **2 + 58 = 60 = N-060**, per batch A 0, B 0, D **0**, E **2**, unbatched **0**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | [x]              |
| 23/08/2026 | N-060                         | **Sitting 6 — Batch B's frontier empties, and the gate that closed it had written its own absence down.** `FORWARD-VOICE.md` Section 5 had said in terms since 20/08 that nothing reads `copier.yml`'s `_exclude`, and nothing read that sentence either. **The population was measured before a line was written**: 83 undeclared Direction B citations across 45 files, 56 of them `copier.yml`, against 24 already marked — and the node's own "12 marked sites" had **doubled inside two days** without anyone noticing, the fourth charted number on this map to age that fast. **Seven of the 83 were never marker candidates.** A bare `SECURITY.md` cited from `code/docs/api-design/`, `architecture/`, `performance/`, `rls/` and `23-pr-and-review/` resolves sibling-first, finds no sibling, and lands on the **excluded root** `SECURITY.md` — five guides and one link label meaning the OWASP guide were pointing at the licence-policy file by accident, repaired to `code/docs/SECURITY.md`; the seventh means an upstream crate's own file and took `ignore`. **The gate found a class the node was not convened for**, which is the argument for building it. **75 lines were marked and the tree proven green before Check 3 was added** — N-025's warning honoured a fourth time. **Q1 was the decision the node did not know it carried**: reading `_exclude` naively would demand `template-only` on every `RUST.md` and `MOBILE.md` citation, which is **false** in any project that took the surface, so surface-gated entries are out of the set and keep their prose `rust-only` flag. The set is **parsed, never listed** — negations, the nine `_tasks` seeds and the regenerated `uv.lock` all subtracted — and the run prints its size, because a parse that found nothing would otherwise report clean. **Proven in both directions and at the derivation**: two fixtures whose every path RESOLVES here so only Check 3 can fire, four direct `is_template_only` assertions, and a live negative control on `CONTEXT.md:129`. **The clause cost ~5s against a 1m41 baseline** once a basename index was added; the naive form cost 33s, measured rather than estimated. **The commit produced a Batch A defect while closing a Batch B node** — a live `<:` opener written into a rendered file, caught by `check-template-tokens.sh` — the second time a settlement on this map created an instance of another class. **The sibling read found a ninth map**, `MAP-CLAUDE-DESIGN-HANDOFF`, never entered in this map's table: five executions, five different failures. **Charted nothing.** Counts recounted from the tables: **6 + 54 = 60 = N-060**, per batch A 0, B **0**, D 1, E 4, unbatched 1                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | [x]              |
...
- [x] **Every node marked "blocking a story" is resolved** — **regressed and restored on
      ...
      **N-032**, which gated not story-writing but _producing a project at all_; **re-ticked the
      ...
      non-blocking by their own wiring (**recounted a fourteenth time 23/08/2026** after sitting 8
      settled N-021 and N-026, taking the figure to **0**; **a thirteenth** after sitting 7 settled
      N-023, N-045, N-049 and N-039, taking it to **2**; **a twelfth time** after sitting 6
      settled N-060, which took the figure to **6**; **recounted an eleventh time 21/08/2026** after sitting 3
...
- [x] Every resolved node links to the artefact it became — all **60** carry a commit, a script, a standing limitation, or
      ...
      **34**. Two settlements, no recount, exactly the failure the rule names. **Charting N-047 to
      ...
      as a resolved row: **14 + 46 = 60 = N-060**. **Sitting 6 took it to 54 and the frontier to
      6, both written in the same pass as the row** — the practice rather than the exception for
      the third sitting running: **6 + 54 = 60 = N-060**. **Sitting 7 took it to 57 and the
      frontier to 3**, closing Batch D, then to 58 and **2** as N-039 closed the unbatched class in
      the same sitting: **2 + 58 = 60 = N-060**. **Sitting 8 took it to 60 and the frontier to
      ZERO**, closing Batch E last: **0 + 60 = 60 = N-060**)
...
- [ ] **Sibling maps read before the next resolve session** — added 16/08/2026. Four live maps
      ...
      **Discharged for sitting 6, 23/08/2026 — five of five, and the failure mode changed a
      fourth time.** All eight listed siblings read, nothing new inbound, `MAP-ABSENCE`'s eight
      still accounted for. What it found is a **ninth map that had existed for some time and was
      never entered here at all** — `MAP-CLAUDE-DESIGN-HANDOFF`, 23 open and 7 blocking. Not a
      missing row on a young table, not a wrong description, not a map written mid-sitting: a map
      nobody had read into the table since it was written. **The duty's own record is now five
      executions and five distinct failures**, which is a stronger argument than any single one
      of them — the table does not decay in one way, so no single check retires it. The read also
      **produced its first inbound correction rather than a self-correction**: the ninth map
      asserts both standing registers are empty shipped skeletons, which sitting 5 falsified.
      Recorded on its row, and left for that map to fix.
