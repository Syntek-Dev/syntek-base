...
**Frontier open**: 19 · **Blocking open**: 0 · **Resolved**: 40 — recounted from the tables
...
| N-031 | Shipped guides cite copier-**excluded** paths — resolves here, dangles in every generated project                                                    | grilling | ~~N-043~~ **cleared 18/08** · ~~N-042~~ **cleared 20/08 — settled, and `Citations resolve` is green at `692ad63`, so this node's PR is no longer blocked at a required check**. **Unblocked, and the last of its blockers gone** | no                |
