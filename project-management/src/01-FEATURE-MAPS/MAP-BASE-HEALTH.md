...
**Frontier open**: 21 · **Blocking open**: 0 · **Resolved**: 38 — recounted from the tables
18/08/2026 after **N-043** settled: **21 + 38 = 59 = N-059**. Per batch:
A 0 · B 5 · D 4 · E **8** · unbatched 4. **N-031 loses a blocker with it** — the rule it classifies
against is now true — leaving `N-042` as its only remaining one, and the three-node batch the map
argued for is now the **pair** N-037 × N-031. Previously 22 · 0 · 37 after N-046 and N-055 settled
...
| N-031 | Shipped guides cite copier-**excluded** paths — resolves here, dangles in every generated project                                                    | grilling | ~~N-043~~ **cleared 18/08 — the rule it classifies against is now true** · **N-042** in practice (its own PR is blocked at a required check)                                     | no                |
