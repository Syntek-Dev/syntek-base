...
**Frontier open**: 2 · **Blocking open**: 0 · **Resolved**: 58 — recounted from the tables
23/08/2026 after **sitting 7** settled **N-023**, **N-045**, **N-049** and **N-039**:
**2 + 58 = 60 = N-060**. Per batch: A 0 · B 0 · D **0** · E **2** · unbatched **0**.
**Four of the five classes now have an empty frontier, and the whole remaining frontier is two
nodes in one batch** — N-021 and N-026, both parked on unmeasured premises and both named a
disposition rather than a sitting since 21/08. **The Destination is two decisions and one
fog-of-war discharge away.**
...
- **`doc-references.sh` now derives what ships, and `is_exempt()` still hardcodes it — two
  ...
  **It stays on that map, on Sam's call**, and the reason is the standing one: settling another
  ...
  design one, only to decide whether `is_exempt()` should call it. Recorded on both maps. Surfaced by N-037, and larger
...
- [x] **Every node marked "blocking a story" is resolved** — **regressed and restored on
      ...
      **N-032**, which gated not story-writing but _producing a project at all_; **re-ticked the
      ...
      non-blocking by their own wiring (**recounted a thirteenth time 23/08/2026** after sitting 7
      settled N-023, N-045, N-049 and N-039, taking the figure to **2**; **a twelfth time** after sitting 6
...
- [x] Every resolved node links to the artefact it became — all **58** carry a commit, a script, a standing limitation, or
      ...
      **34**. Two settlements, no recount, exactly the failure the rule names. **Charting N-047 to
      ...
      the same sitting: **2 + 58 = 60 = N-060**)
