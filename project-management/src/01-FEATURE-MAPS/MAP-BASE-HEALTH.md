...
**Frontier open**: 3 · **Blocking open**: 0 · **Resolved**: 57 — recounted from the tables
23/08/2026 after **sitting 7** settled **N-023**, **N-045** and **N-049**:
**3 + 57 = 60 = N-060**. Per batch: A 0 · B 0 · D **0** · E **2** · unbatched 1.
...
| Node  | Decision                                                                                                          | Type     | Blocked by | Blocking a story? |
| ----- | ----------------------------------------------------------------------------------------------------------------- | -------- | ---------- | ----------------- |
| N-039 | **Residue deleted 23/08/2026; open on its GATE half alone** — which audit owns a transcript-residue check, if any | grilling | none       | no                |
...
**N-039 — SETTLED IN PART 23/08/2026, sitting 7, and what remains is the half it deferred.**
...
- **So the decision is three-way and none of it is a sweep:** widen `conflict-markers.sh` in place
  and accept a name narrower than its contents; rename it and re-point the flip; or write a
  one-grep `transcript-residue.sh` and add a twenty-third context. **Re-typed `grilling`** — the
  node was `task` on the premise that only a deletion was owed, and the deletion is done.
- **It is also the first node to reach the taxonomy question with the trigger already fired**
  (see _Fog of war_): "does a one-line grep join the audit set" is that question at one instance.
...
- [x] **Every node marked "blocking a story" is resolved** — **regressed and restored on
      ...
      **N-032**, which gated not story-writing but _producing a project at all_; **re-ticked the
      ...
      settled N-023, N-045 and N-049, taking the figure to **3**; **a twelfth time** after sitting 6
...
- [x] Every resolved node links to the artefact it became — all **57** carry a commit, a script, a standing limitation, or
      ...
      **34**. Two settlements, no recount, exactly the failure the rule names. **Charting N-047 to
      ...
      frontier to 3**, closing Batch D: **3 + 57 = 60 = N-060**)
