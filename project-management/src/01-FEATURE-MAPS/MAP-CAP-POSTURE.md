...
**Status**: **Charted and resolved** — the frontier is empty; all seven slices are cuttable
...
- **Whether the posture register and the timeout register are one artefact.**
  `MAP-RETRY-AND-IDEMPOTENCY` N-012's register (client · connect · read · retry owner · where
  set) is unbuilt; a merged answer sheet may be right, but S-01 there has to land first.
- **Multi-node Valkey.** Quorum, replication and split-brain territory — deferred behind the
  (phantom) cache-posture ADR; becomes a node only if a cache-posture decision is ever charted.
- **The deploy repo's actual behaviour on `degraded`/`down`** — external and unreadable from
  here; the contract assigns all consumption downstream (`HEALTH-CONTRACT.md:58`).
- **PACELC beyond slow-is-absent.** Q5 settled the rule; with N-009's split settled (each home
  states its own clause), teaching the EL half explicitly is S-01 story work, not a node.
- **What presence means.** N-016 settled its _posture_ (soft state on the pub/sub plane, lost
  on outage, reconstructed by the next heartbeat) but nothing in the tree defines the feature —
  online/away/offline semantics, who may see whom, the staleness window. No story, no doc, no
  code owns it; inventing it here would be a decision made on nothing.
