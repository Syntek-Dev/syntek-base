...
**N-006 is reopened as N-026, 21/08/2026**: the no-ADR decline rested on `14-DECISIONS/` shipping
...
| N-006 | ~~**No ADR.**~~ **REOPENED as N-026, 21/08/2026** — the outcome may stand, but its stated reason (`14-DECISIONS/` ships) was measured false. The Tier A exception mechanism is unaffected                            | grilling | 15/08/2026 | **superseded by N-026**; the exception grep still → **N-016**  |
...
not inferred:** a probe file was placed at `src/14-DECISIONS/ADR-001-PROBE.md`, a real
`copier copy` was run to a scratch directory, and the generated project's `14-DECISIONS/` held
...
| N-026 | **Reopen N-006 — does the no-ADR decline survive?** Its mechanical premise was measured false 21/08/2026: `14-DECISIONS/` **is** copier-excluded, proved by probe. Decide whether the decline stands on the house-pattern grounds alone, and whether that reaches the four other maps citing the same reason | grilling | none — **takeable**                    | no                |
...
| 21/08/2026 | **N-006's premise re-tested against the live `copier.yml`**            | **Failed.** Probe ADR placed, real `copier copy` run, generated `14-DECISIONS/` held only the pair + `ADR-000-TEMPLATE.md`. Reopened as N-026; four sites struck                                                              |
