...
| `N-010` | Pin copier exactly at both live sites; raise the declared floor                                                      | task     | 27/08/2026 | `S-05` — two edit sites named below                             |
...
| `N-017` | **Widened** to "self-tests that fail downstream" — two scripts, not one                                              | task     | 27/08/2026 | `S-03` acceptance criteria                                      |
...
| `N-019` | An explicit `lefthook install` in `install.sh`, keeping the guard                                                    | task     | 27/08/2026 | `S-03` acceptance criterion                                     |
...
| `N-031` | Make the path exist — glob the root rule, track a nested `.gitignore` admitting the pair                             | task     | 28/08/2026 | `S-07` acceptance criterion; edit sites below                   |
...
| `N-013` | Reword `:559` so the examples read as examples — the existing naming-row guard then covers them                      | task     | 28/08/2026 | `S-07` acceptance criterion (from retired `S-06`)               |
| `N-026` | Correct all four toward `06-GENERATION.md:140`'s own wording                                                         | task     | 28/08/2026 | `S-08` acceptance criterion — forced by `N-005`                 |
| `N-027` | **Moot** — `N-019` makes the claim true rather than needing correction                                               | task     | 28/08/2026 | Retired unbuilt; `S-03` acceptance criterion                    |
| `N-024` | Read the report line, state the ignore count, fail closed when it hits zero                                          | task     | 28/08/2026 | `S-10` slice row; acceptance criteria                           |
| `N-029` | A `--self-test` step in each gate's existing workflow, matching `audit-conflict-markers.yml:43`                      | task     | 28/08/2026 | `S-10` acceptance criterion                                     |
| `N-033` | `static-analysis.sh:398` counts `len(results)` while `:395` sends errors to the body — same principle, own mechanics | task     | 28/08/2026 | `S-10` acceptance criterion; graduated from fog of war          |
| `N-018` | Recompute as **our** digest over all 15 files, stated basis, asserted in `skill-conformance.sh`                      | task     | 28/08/2026 | `S-05` acceptance criterion                                     |
...
| `N-021` | The **bare** directory form, matching the three surface flags — `/**` prunes, bare removes                           | task     | 28/08/2026 | `S-08` acceptance criterion                                     |
| `N-030` | Tiered `timeout-minutes` from measured CI p95s, swept now at an interim 30; enforced by a shipping `run:` step       | task     | 28/08/2026 | `S-08` acceptance criterion                                     |
...
| ------ | ------- | --------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `S-03` | `US###` | The copier-config-reading scripts             | Backend: no · QA: audit self-tests (x2), pre-commit probe, generation probe on **both** answer sets, **the `.copier/README.md` lefthook claim re-read as true** · Nodes: N-006 ✅, N-017 ✅, N-019 ✅, N-027 (moot)                                  |
| `S-05` | `US###` | The gate-side rule, and its first use         | Backend: no · QA: `skill-conformance` self-test, generation probe, `prettier --check` · Nodes: N-008 ✅, N-018 ✅, N-016 ✅, N-004 ✅, N-015 ✅, N-010 ✅ (N-009 moot)                                                                               |
| `S-07` | `US###` | Shipped documents that promise absent paths   | Backend: no · QA: `doc-references.sh` + a new naming-row self-test probe, `sync-trees.sh` pre-commit probe, `shipped-readme.sh`, a nine-`.gitignore` tracking probe · Nodes: N-014 ✅, N-023 ✅, N-031 ✅, N-013 ✅                                  |
| `S-08` | `US###` | The format pass, and the gate that watches it | Backend: no · QA: both-pole generation probe, the three commands, `timeout-minutes`, byte-identity vs the reference pole · Logging: CI annotations · Nodes: N-005 ✅, N-007 ✅, N-011 ✅, N-012 ✅, N-026 ✅, N-022 ✅, N-025 ✅, N-021 ✅, N-030 ✅ |
| `S-09` | `US###` | The reports tree's documentation pair         | Backend: no · QA: `docs-pairing.sh` (both gate edits), `sync-trees.sh --check`, generation probe · Nodes: N-032 ✅                                                                                                                                   |
| `S-10` | `US###` | Every audit's verdict is legible              | Backend: no · QA: `security.sh` on both poles, the two new `--self-test` steps, `static-analysis.sh --self-test`, a declared-ignore drift probe · Nodes: N-024 ✅, N-029 ✅, N-033 ✅, N-020 ✅, N-028 ✅                                            |
