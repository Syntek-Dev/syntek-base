...
| ------ | ------- | --------------------------------------------- | ---------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `S-03` | `US###` | The copier-config-reading scripts             | N-006 ✅, N-017 ✅                                                                       | TBD                                                                                                                                                                                                                                                                                                                                                                                 | Backend: no · QA: audit self-tests (x2) — **ships after `S-05`**, which builds the `_lib/template-tree.sh` idiom `N-006`'s idiom half was sent to at `N-008`                                                                                        |
...
| `S-11` | `US009` | The hooks arm on purpose                      | N-019 ✅                                                                                 | An explicit `lefthook install` step lands in `install.sh` after the JavaScript-dependency step, guarded on `.git` and hard-failing at exit 2 otherwise; `--ignore-scripts` stays on all three `install-frontend.sh` branches and `package.json`'s `prepare` is untouched; `.copier/README.md:441`'s claim becomes true **without being edited** (`N-027`, moot and retired unbuilt) | Security: supply-chain — the `--ignore-scripts` control asserted intact, the hook-arming moment made explicit · QA: integration, manual — pre-commit probe, generation probe on **both** answer sets, the `.copier/README.md` claim re-read as true |
...
**`S-03` is split, and `S-11` is the half that was ready (17/09/2026, at `02-story-creation`).**
As charted `S-03` carried four nodes across two halves that turned out not to need each other,
and only one of them was takeable:

- **`N-006` + `N-017` stay on `S-03`, and it is no longer the first slice.** `:144` sends
  `N-006`'s **idiom half to `N-008`**, and `N-008` settles that idiom as "a shared
  `_lib/template-tree.sh` … across 28 core scripts" covering the unguarded copier reads
  (`:256-259`). That file does not exist and `N-008` lives in **`S-05`**. Cutting `S-03` first
  would write three ad-hoc guards `S-05` then rewrites — a **second** instance-before-class,
  against this batch's own ruling at `:397-398` that it accepts one.
- **`N-019` + `N-027` became `S-11`**, and carry the priority argument intact. The hooks are not
  installed at generation, so nothing is blocked on a fresh project; they arm on the first
  non-suppressed `pnpm` invocation, which makes the landmine live at an unpredictable moment
  rather than never. That is still the only defect here that _stops work_ once it fires, and
  `N-019`'s own fix — making the install deliberate — arms it on purpose. It needs nothing from
  `_lib/`, shares no file with the retained half, and was cut as **`US009`**.

**The split follows `S-09`'s precedent (`:325`) — a new appended number, never a suffix.**
`N-027` does not travel as a node: it is **moot and retired unbuilt** (`:159`), so `S-11` names
`N-019` alone and satisfies `MAP-000-TEMPLATE.md:78` without a notation fix. `.copier/README.md`
is not edited by `US009`; its claim becomes true because the step it describes starts existing.
Its line has drifted since `:326` was written — `:436` then, **`:441`** today, re-measured
17/09/2026.
...
| 17/09/2026 | — (no node settled)                     | **`S-03` split at `02-story-creation`; `S-11` cut as `US009`.** No node was settled, reopened or re-dispositioned — this is a slice-boundary change only. `N-006`+`N-017` stay on `S-03`, which now waits on `S-05` because `N-006`'s idiom half was sent to `N-008` (`:144`, `:256-259`) and `_lib/template-tree.sh` does not exist; `N-019` moved to the new `S-11` with `N-027`'s moot disposition as its acceptance criterion. Precedent: `S-09`'s split from `S-07` (`:325`) — a new appended number, never a suffix. Frontier unchanged because it was already empty on 28/08/2026 | n/a — frontier already empty |
