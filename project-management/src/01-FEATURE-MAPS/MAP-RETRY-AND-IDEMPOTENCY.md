...
**Status**: Fully charted (01/09/2026) — cutting; `S-01` → `US001`, `S-02` → `US005` (04/09/2026)
**Frontier open**: 0 · **Blocking open**: 0 · **Resolved**: 25 · **Slices**: 9 (S-09 split from
S-02 at `02-story-creation`, 04/09/2026)
...
| N-008 | One layer retries; SDKs clamped; delegation via register    | grilling | 01/09/2026 | S-02 → `US005` + `ADR-US005-ONE-LAYER-…`  |
...
| N-012 | Timeout register — `how-to/src/`, every outbound socket     | grilling | 01/09/2026 | S-09 slice row + decision below           |
...
`retries={"total_max_attempts": 1}` and kin — **corrected 05/09/2026**, this node read
`max_attempts: 1`, which counts retries in a `Config` object and so permits two attempts; see
`project-management/src/15-DECISIONS/ADR-US005-ONE-LAYER-DECIDES-TO-RETRY-04-09-2026.md`);
a client that keeps transport retries does so via its
...
| S-02  | `US005` | Retry doctrine: ownership, `Retry-After`, budgets, breaker         | `N-008`–`N-011`             | Retry guide states the single-owner rule (SDKs clamped, delegation via a register row), the clamped `Retry-After` rule (`max(backoff, Retry-After)`, never past the total-age ceiling), the Celery-default budget table with its derived worst-case column and the webhook override, and the breaker deferral with both triggers; `SERVICE-AND-MIDDLEWARE.md:265` and `NEGATIVE-SPACE.md:226` reduce to pointers; **four** budget contradictions gone — the two `max_retries=3` sites land ~3 min and ~9 min apart, so the count is of worst cases, not statements                                                                                                  | Security: retry amplification · QA: manual                                              |
...
| S-09 | `US###` | The outbound timeout register | `N-012` | `how-to/src/OUTBOUND-TIMEOUTS.md` ships on the `PLATFORM-PROVIDERS.md` pattern — rule in the reliability family, values per project — with one row per outbound socket across five columns (client · connect · read · retry owner · where set): HTTP clients, Valkey, Postgres, the broker and the Sentry transport. The 5 s/30 s pair survives as the shipped generic-HTTP default and `performance/API-AND-MONITORING.md:33-34` reduces to a pointer. Registered on the three index surfaces that today list four sheets. The red rows are already measured: Valkey carries no `SOCKET_TIMEOUT` (the `TIMEOUT: 300` beside it is cache TTL, not a deadline), Postgres no connect timeout, the ClickUp `urlopen` no `timeout=` and no CI `timeout-minutes`, and the one bounded call uses `timeout=15` — a third value matching neither default. Per-provider numeric SLOs stay out of scope | Security: outbound timeouts · QA: manual |
...
**S-09 split out of S-02 at `02-story-creation`, 04/09/2026.** S-02 as charted carried five
deliverables — four prose rules plus a whole new `how-to/src/` answer sheet with roughly twenty
measured rows across three index surfaces — which prices as an epic rather than a story against
the ladder that gave `US004` an 8 for one script and its fixtures. The cut is at the only seam
that is real: **`N-008`–`N-011` write into the family `S-01` must create first, whereas `N-012`'s
register is standalone and measurable against today's tree.** So S-02 narrows to the doctrine and
S-09 carries the register, each with its own flag manifest. The two share no file and neither
blocks the other. This is the one re-cut `02-story-creation` performed rather than deferring to
`01-feature-map` Step 8a, on the reasoning that a slice row whose acceptance describes a
deliverable no story carries is exactly the drift the folder's _index, not vault_ rule exists to
stop.

**S-02's acceptance sharpened from three budget contradictions to four**, same session. The two
`max_retries=3` sites — `performance/API-AND-MONITORING.md:57` and
`project-management/docs/gdpr/COMPLIANCE.md:27` — look identical and were counted as one, but the
first sets `default_retry_delay=60` (~3 min) and the second sets none and inherits Celery's
3-minute default (~9 min). The charted count was of statements; the defect is a count of worst
cases. A story sharpens its cell and never loosens it.

**S-02 also widened to the whole `gdpr/COMPLIANCE.md:22-49` fence**, settled the same session.
That fence was claimed twice — S-02 owns the `max_retries=3` budget at `:27`, S-04 the
bare-`except` retry shape at `:46-48`. `US005` repairs it whole, so **S-04's "four bad examples"
becomes three**; S-04's row is updated by that story's implementation, not here.
...
**S-09 gates on nothing** — the register is written against the current tree, not against the
family — though `S-05`'s `outbound-timeouts.sh` gates on it, and `S-06` fixes two of its red rows.
