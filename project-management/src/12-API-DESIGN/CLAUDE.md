...
- **Routing:** never write here free-hand — `PLANNING/` designs come from
  `workflows/12-api-design/` (after `src/03-DATABASE/` schema is agreed, before
  `src/14-SPRINT-PLANS/`); `IMPLEMENTATION/` verifications from `workflows/20-pr-and-review/`.
...
- **Concrete steps:** pick the phase → copy that folder's `US000-TEMPLATE.md` to
  ...
  → satisfy the workflow `CHECKLIST.md`. The design feeds `workflows/17-api-code/`.
...
- **Documentation only** — no implementation code (`code/src/django/`), no migration SQL
  or ERDs (`src/03-DATABASE/`), no user stories (`src/01-STORIES/`).
