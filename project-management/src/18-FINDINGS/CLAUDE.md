...
- **Model:** Opus — recording an observed divergence and its smallest fix is an
  ...
  decision escalates to the `planner` agent and graduates to `../13-DECISIONS/`.
- **Concrete steps:** copy `FINDING-US000-TEMPLATE.md` →
  ...
  (`../01-STORIES/US###.md`) and its plan (`../15-STORY-PLANS/STORY-PLAN-US###-*.md`) →
...
- **Record, never fix.** A findings record states the smallest fix; it does not apply it.
  The fix lands in a later story, a `../19-BUGS/` report, or `../20-REFACTORING/`.
