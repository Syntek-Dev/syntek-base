# project-management/src/18-FINDINGS
...
code → 16–20 records**. This folder is a **record** (16-TESTS, 17-REVIEWS, **18-FINDINGS**,
19-BUGS, 20-REFACTORING) — written _after_ code exists, closing the loop on the story plan
...
| Folder           | Answers                                                  |
| ---------------- | -------------------------------------------------------- |
| `17-REVIEWS`     | Is this PR fit to merge?                                 |
| `18-FINDINGS`    | What did shipping this story reveal about the standards? |
| `19-BUGS`        | What is broken, and how was it fixed?                    |
| `20-REFACTORING` | What was restructured, without behaviour change?         |
...
`project-management/workflows/19-implementation-documentation/` — after the code and its
...
- `../01-STORIES/` — the stories findings are anchored to
- `../15-STORY-PLANS/` — the code master a finding closes the loop on, and the next plan it feeds
- `../17-REVIEWS/` — the merge-gating review from the same PR
- `../19-BUGS/` — where a finding that is a defect is escalated
- `../20-REFACTORING/` — where a finding that is structural debt is actioned
- `../13-DECISIONS/` — where a finding that settles a hard-to-reverse trade-off graduates
