...
1. **Clarify the mission.** Read `learning/<topic>/MISSION.md` if it exists; if it is missing or
   ...
   _process/PM_ (agile, stories, sprints, plans, ADRs) or _stack/coding_ (HTMX, Alpine, vanilla
...
4. **Run the lesson — recall, then build.** Teach the concept, then close the loop:
   ...
   - **Build to learn** — {{DEVELOPER_NAME}} drafts a small throwaway artefact under `LESSONS/`: a _process_
     sample (a practice `US###` or `PLAN`) or a _coding_ example run through the real task-runner
