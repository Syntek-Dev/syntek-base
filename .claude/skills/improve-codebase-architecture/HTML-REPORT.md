...
The diagrams carry the weight. Prose is sparse, plain, and uses the glossary terms (from the `codebase-design` skill — `.claude/skills/codebase-design`) without ceremony.
...
Plain English, concise — but the architectural nouns and verbs come straight from the `codebase-design` skill (`.claude/skills/codebase-design`). Concision is not an excuse to drift.
