...
- **Grill first:** any substantial operational task — restructuring a guide or workflow —
  opens with a grilling pass (the owning agent loads `.claude/skills/grill-with-docs`, which owns the round shape and question format) before the artefact is produced; only trivial/mechanical touches skip
