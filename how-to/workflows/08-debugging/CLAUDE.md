...
# CLAUDE.md — how-to/workflows/08-debugging/
...
- **Routing:** environment/operational failure → `global-workflow` skill via
  ...
  healthy, hand off to `code/workflows/10-debug/` (code-logic) or
  `code/workflows/09-debugging-with-logs/` (staging/prod observability).
...
- **Scope boundary:** this workflow diagnoses the _environment_. Code-logic bugs and
  regression tests belong to `code/workflows/10-debug/`; do not fix product code here.
