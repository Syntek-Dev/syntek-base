...
- **Routing:** all substantive work here enters through the `scale-planning` skill
  (`/scale-planning`) and its `scale-planner` agent. Decisions are charted on
  the project's plans folder as `MAP-SCALE-PLANNING.md` (wayfinder) and settled
...
- **Hand-written via the skill:** all seven documents. Nothing is generated by
  tooling; "regenerated" means re-derived from live code by the agent.
