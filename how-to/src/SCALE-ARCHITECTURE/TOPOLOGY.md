...
| Valkey                   | Bare-metal NixOS, loopback; proxies :6501 (DB 0) / :6502 (DB 1) | **Stateful** — cache reconstructible, **broker queue is not** | Single instance; multi-node is later territory in the project's cache-posture ADR     | Broker queue, pub/sub channels, cache, session cache tier, presence keys, rate-limit counters |
...
**Eviction is a durability question, not only a hit-rate one.** The cache tolerates eviction by
design; the broker queue does not, and losing a queued message is silent. The two are not
separated today — `SERVER-ARCHITECTURE/COMPUTE-ALLOCATION.md` records that cache and broker share
DB 0 until a second URL variable is introduced — so the cache's eviction policy currently governs
queued work as well. **Requirement on the deploy repo:** either an eviction policy that cannot
reach broker keys, or a broker instance of its own. Whether an instance-wide policy can evict
across the numbered databases once the split lands is a property of the deployed Valkey build, so
confirm it there rather than inferring it here. The task-side statement of the same requirement is
`code/docs/TASK-AUTHORING.md` → _Idempotency_, which also carries the acknowledgement settings
that decide whether a delivered message survives a worker's death.
