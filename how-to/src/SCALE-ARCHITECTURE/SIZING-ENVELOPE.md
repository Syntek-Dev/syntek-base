...
| Setting                                      | Where                                                                             | Baseline                                                                                                                                                                                                                                                                                                                    |
| -------------------------------------------- | --------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `maxmemory` + `maxmemory-policy allkeys-lfu` | Valkey NixOS config — **deploy-repo knob** (the cache-stampede-mitigation ADR)    | `maxmemory` **TBD — regenerate**; policy `allkeys-lfu` — **the deploy repo's value, unverified from here.** It must not be able to evict broker keys (`code/docs/TASK-AUTHORING.md` → _Idempotency_; `TOPOLOGY.md` → _Where connections and state actually live_), which is not satisfied while cache and broker share DB 0 |
