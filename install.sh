...
generate_machine_spec() {
  ...
  mkdir -p "$(dirname "$SPEC_FILE")"
  {
    ...
    # — killing this generator on every base-template run. Doubling the percent signs is
    # printf's other escape and is fatal here: Copier renders this file, and a doubled opener
    # is not a token but a Jinja syntax error that stops generation of every project dead.
    # Passing the slug as an argument is the only fix both parsers accept.
